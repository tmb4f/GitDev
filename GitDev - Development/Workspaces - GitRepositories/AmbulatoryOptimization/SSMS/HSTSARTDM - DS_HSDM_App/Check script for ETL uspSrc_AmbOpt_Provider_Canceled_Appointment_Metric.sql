...
--		   02/05/2025 - TMB - use Rptg vwDim_Clrt_SERsrc Prov_Nme value for current provider name
--		   06/17/2025 - TMB - add Prov_Typ = 'Licensed Clinical Social Worker' to app_flag designator
--************************************************************************************************************************
...
(
	...
	(
		...
		( --main
			...
			--AND appts.DEPARTMENT_ID = 10242012 -- UVPC FAMILY MEDICINE
			--AND appts.PROV_TYPE_OT_NAME = 'Resident'
...
SELECT
	       ...
	       ,CASE WHEN tabrptg.Prov_Typ IN ('NURSE PRACTITIONER','PHYSICIAN ASSISTANT','NURSE ANESTHETIST','CLINICAL NURSE SPECIALIST','GENETIC COUNSELOR','AUDIOLOGIST','LICENSED CLINICAL SOCIAL WORKER') THEN 1 ELSE 0 END AS app_flag
	  ...
	  tabrptg.organization_id
	  ,tabrptg.organization_name
	  ,tabrptg.service_id
	  ,tabrptg.service_name
	  ,tabrptg.clinical_area_id
...
WHERE event_category = 'Aggregate'
--AND (Prov_Typ = 'Fellow' OR Prov_Typ = 'Nurse Practitioner' OR Prov_Typ = 'Physician' OR Prov_Typ = 'Physician Assistant')
AND tabrptg.Prov_Typ = 'Audiologist'
