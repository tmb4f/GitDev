...
SELECT
	ddte.year_num,
	SUM(accepted) AS Numerator,
	SUM(incoming_transfer) AS Denominator
FROM #xtr xtr
LEFT OUTER JOIN DS_HSDW_Prod.Rptg.vwDim_Date ddte
	ON ddte.day_date = xtr.event_date
WHERE 1 = 1
AND incoming_transfer = 1
GROUP BY
	year_num
...
SELECT
	ddte.year_num,
    COALESCE(SUM(   CASE
						WHEN xtr.accepted = 0 THEN 1
						ELSE 0
					END
				)
			,0
			)                                       AS numerator, --in case of null return zero
	COALESCE(COUNT(xtr.event_count), 0)              AS denominator
FROM #xtr xtr
LEFT OUTER JOIN DS_HSDW_Prod.Rptg.vwDim_Date ddte
	ON ddte.day_date = xtr.event_date
WHERE 1 = 1
