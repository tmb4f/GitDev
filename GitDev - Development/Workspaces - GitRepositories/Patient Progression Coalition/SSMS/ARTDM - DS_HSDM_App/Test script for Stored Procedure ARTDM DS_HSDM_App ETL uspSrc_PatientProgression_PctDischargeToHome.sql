...
IF OBJECT_ID('tempdb..#dsch ') IS NOT NULL
DROP TABLE #dsch
...
( --prov_team_dc--prov team at discharge
    ...
    ( --ppt_dc
        ...
        --FROM CLARITY..EPT_TEAM_AUDIT              AS prteam
        FROM DS_HSDW_Prod.Rptg.vwFact_Clrt_EPT_TEAM_AUDIT              AS prteam
            --INNER JOIN CLARITY..CLARITY_EMP       AS prov_emp
            --    ON prteam.TEAMAUDIT_USER_ID = prov_emp.USER_ID
            INNER JOIN DS_HSDW_Prod.Rptg.vwDim_Clrt_EMPlye       AS prov_emp
                ON prteam.sk_Dim_Clrt_EMPlye = prov_emp.sk_Dim_Clrt_EMPlye
            --INNER JOIN CLARITY..CLARITY_SER       AS prov_ser
            --    ON prov_emp.PROV_ID = prov_ser.PROV_ID
            INNER JOIN DS_HSDW_Prod.Rptg.vwDim_Clrt_SERsrc       AS prov_ser
                ON prov_emp.EMPlye_PROV_ID = prov_ser.PROV_ID
            --INNER JOIN CLARITY..PROVTEAM_REC_INFO AS team
            --    ON prteam.TEAM_AUDIT_ID = team.ID
        ...
        WHERE prteam.PRIMARYTEAM_AUDI_YN = 'Y' --primary prov team
              AND prteam.PAT_ENC_CSN_ID = dsch.PAT_ENC_CSN_ID
    ) AS ppt_dc
    WHERE dsch.Dsch_Dtm
