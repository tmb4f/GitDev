...
--ALTER PROCEDURE [ETL].[uspSrc_AmbOpt_Dentistry_RecommendProvOffice]
--    (
...
/**********************************************************************************************************************
WHAT: Ambulatory Optimization Reporting:  Dentistry Likelihood Recommending Practice (Metric Id ???)
WHO : Joe Mouton
WHEN: 06/15/2022
WHY : Press Ganey Dentistry results for survey question:
      "Likelihood of your recommending this dental office to others"
-----------------------------------------------------------------------------------------------------------------------
INFO:                
      ...
      OUTPUTS:  ETL.uspSrc_AmbOpt_Dentistry_RecommendProvOffice
--------------------------------------------------------------------------------------------------------------------------
MODS: 	
      06/15/2022 - JBM - created stored procedure
		...
		LEFT OUTER JOIN Stage.Scheduled_Appointment appts
				...
				LEFT OUTER JOIN DS_HSDW_Prod.Rptg.vwRef_Physcn_Combined physcn
				    ON physcn.sk_Dim_Physcn = CASE
												...
												ELSE -999
										      END
		WHERE   resp.Svc_Cde='DS' AND resp.sk_Dim_PG_Question IN ('2948') -- Likelihood of your recommending this dental office to others
...
ON rec.day_date=pm.RECDATE
 ...
 --          sk_Dim_Physcn
	...
	--   SELECT *
	--FROM #ds
	----WHERE epic_department_id = 10210069
	----ORDER BY event_category
	----        ,epic_department_id
	----        ,event_date
	----ORDER BY event_category
...
/*
    SELECT organization_name
	              ...
	              ,epic_department_name
				  ,weighted_score
				  ,CASE WHEN event_count = 1 THEN 1 ELSE 0 END AS LklihdRecommendOfficeResponse
				  ,ambulatory_flag
...
*/
	--SELECT *
	--FROM #summary
	--WHERE LklihdRecommendOfficeResponse = 1
...
/*
	SELECT
				  ...
				  --,SUM(LklihdRecommendOfficeResponse) AS LklihdRecommendOfficeResponse
				  ,SUM(LklihdRecommendOfficeResponse) AS Responses
	FROM #summary
	WHERE LklihdRecommendOfficeResponse = 1
	--AND ambulatory_flag = 1
	--AND SUBSTRING(Survey_Designator,1,2) = 'DS'
	GROUP BY 
	               organization_name
	              ,service_name
				  ,clinical_area_name
