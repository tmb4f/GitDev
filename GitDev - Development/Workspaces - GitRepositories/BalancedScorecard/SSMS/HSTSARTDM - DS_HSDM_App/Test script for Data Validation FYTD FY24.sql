...
WHERE
ambulatory_flag = 1
--(ambulatory_flag = 1 and community_health_flag = 0)
...
--AND (rpt.ambulatory_flag = 1 and rpt.community_health_flag = 0)
...
WHERE event_category = 'Aggregate'
--AND (Prov_Typ = 'Fellow' OR Prov_Typ = 'Nurse Practitioner' OR Prov_Typ = 'Physician' OR Prov_Typ = 'Physician Assistant')
AND (tabrptg.AMB_Scorecard_Flag = 1 AND tabrptg.Prov_Typ <> 'Fellow')
AND ambulatory_flag = 1
--AND (ambulatory_flag = 1 and community_health_flag = 0)
...
C:\Users\tmb4f\source\Workspaces\Tom Burgan - My Files\Development\GitRepositories\BalancedScorecard\SSMS\HSTSARTDM - DS_HSDM_App\Check script for Tableau Workbook FYTD Inpatient HCAHPS Top Box Score.sql
*/
...
SELECT
	...
	SUM(weighted_score) AS weighted_score,
	SUM(event_count) AS StaffWorkedTogetherResponse
FROM DS_HSDM_App.TabRptg.Dash_AmbOpt_INStaffWorkedTogether_Tiles tabrptg
LEFT OUTER JOIN DS_HSDM_App.Mapping.Epic_Dept_Groupers g
ON g.epic_department_id = tabrptg.epic_department_id
