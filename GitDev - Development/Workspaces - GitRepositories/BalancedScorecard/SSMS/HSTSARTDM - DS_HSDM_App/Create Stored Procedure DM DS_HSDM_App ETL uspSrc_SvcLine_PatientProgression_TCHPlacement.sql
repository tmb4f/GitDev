...
--EXEC [DS_HSDM_App].[ETL].[uspSrc_SvcLine_PatientProgression_TCHPlacement]

CREATE PROCEDURE [ETL].[uspSrc_SvcLine_PatientProgression_TCHPlacement]
    (
...
--/**********************************************************************************************************************
--WHAT: Create procedure [ETL].[uspSrc_SvcLine_PatientProgression_TCHPlacement]
--WHO : Tom Burgan
--WHEN: 7/8/2019
--WHY : for TCH placements : Time interval between Referral and Acceptance
--       
-------------------------------------------------------------------------------------------------------------------------
--INFO: 
--      INPUTS:	HSTSARTDM DS_HSDM_App Rptg.vwCuraspan_By_Discharge
--              
--------------------------------------------------------------------------------------------------------------------------
--MODS: 	
--         07/10/2019 -TMB - create stored procedure
--       
--************************************************************************************************************************
...
SELECT DISTINCT
 ...
 --flags	
		CAST('TCH_Placement' AS VARCHAR(50))				'event_type'
...
--patient info
	   ...
	   ,CAST(CASE WHEN FLOOR((CAST (COALESCE(dsch.dsch_date_time,
									   day_date) AS INTEGER)
					   - CAST(CAST(dsch.BIRTH_DATE AS DATETIME) AS INTEGER))
					   / 365.25) < 18 THEN 1
			      ELSE 0
...
--Provider/scheduler info
       ,dsch.dsch_atn_prov_provider_id						'provider_id'
       ,dsch.dsch_atn_prov_provider_name					'provider_name'
       ,dsch.dsch_atn_prov_physcn_Service_Line				'prov_service_line'  --service line

 --fac-org info
	   ,dsch.dsch_DEPARTMENT_ID								'epic_department_id'
       ,dsch.dsch_Clrt_DEPt_Nme								'epic_department_name'
	   ...
	   ,physcn.SOM_division_id								'som_division_id'
...
FROM DS_HSDW_Prod.dbo.Dim_Date dmdt
		LEFT OUTER JOIN  (SELECT	   [PAT_ENC_CSN_ID]
							...
							--AND ((hscdc.Provider_Name LIKE 'UVA Encompass%')
							--     OR (hscdc.Provider_Name LIKE 'UVA Transitional%'))
							AND (hscdc.Provider_Name LIKE 'UVA Transitional%')
							UNION ALL
						  SELECT	   cbd.[Patient Account Number] AS PAT_ENC_CSN_ID
							...
							--AND ((CASE WHEN cbd.[Provider Name]  <> '' THEN cbd.[Provider Name] ELSE NULL END LIKE 'UVA Encompass%')
							--     OR (CASE WHEN cbd.[Provider Name]  <> '' THEN cbd.[Provider Name] ELSE NULL END LIKE 'UVA Transitional%'))
							AND (CASE WHEN cbd.[Provider Name]  <> '' THEN cbd.[Provider Name] ELSE NULL END LIKE 'UVA Transitional%')
							
						 ) hspc													    ON CAST(dmdt.day_date AS DATE)=CAST(hspc.[Referral_Dttm] AS DATE)					
		LEFT OUTER JOIN DS_HSDW_App.Rptg.PatientDischargeByService_v2 dsch			ON dsch.PAT_ENC_CSN_ID = CAST(hspc.PAT_ENC_CSN_ID AS NUMERIC(18,0))															/*Data below not yet in Pt Disch table*/
		LEFT OUTER JOIN DS_HSDW_Prod.Rptg.vwFact_Pt_Enc_Clrt enc					ON dsch.sk_Fact_Pt_Enc_Clrt = enc.sk_Fact_Pt_Enc_Clrt		--linking table
		LEFT OUTER JOIN DS_HSDW_Prod.Rptg.vwDim_Patient patient						ON dsch.sk_Dim_Pt = patient.sk_Dim_Pt						--patient name (combined) and sex

        LEFT OUTER JOIN DS_HSDW_Prod.Rptg.vwRef_MDM_Location_Master_EpicSvc locsvc	ON locsvc.epic_department_id = dsch.dsch_DEPARTMENT_ID 
	...
	-- Identify transplant encounter--
        ...
        LEFT OUTER JOIN (SELECT DISTINCT
					       ...
					       ,enc.sk_Dim_Pt
					     FROM
					       DS_HSDM_Prod.Rptg.Big6_Transplant_Datamart btd
					     ...
					     WHERE
					       (btd.TX_Episode_Phase = 'transplanted'
					        AND btd.TX_Stat_Dt >= @startdate
					        AND btd.TX_Stat_Dt < @enddate)
					        AND btd.TX_GroupedPhaseStatus = 'TX-ADMIT') tx ON dsch.sk_Fact_Pt_Enc_Clrt = tx.sk_Fact_Pt_Enc_Clrt
                ...
                (
                    ...
                    ON dsch.dsch_DEPARTMENT_ID = mdmloc.EPIC_DEPARTMENT_ID

				LEFT OUTER JOIN DS_HSDW_Prod.Rptg.vwRef_Physcn_Combined physcn		ON physcn.sk_Dim_Physcn = dsch_atn_prov_sk_physcn
WHERE 1=1
