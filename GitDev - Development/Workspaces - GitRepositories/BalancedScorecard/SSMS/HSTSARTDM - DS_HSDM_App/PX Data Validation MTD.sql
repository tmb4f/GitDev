...
SET @startdate = '12/1/2023 00:00 AM'
...
SET @enddate = '12/31/2023 11:59 PM'
...
ON resps.epic_department_id = CAST(TRIM('''' FROM IT_DEPT_ID) AS NUMERIC(18,0))
  WHERE Received_Date <> 'Total'
  AND IT_DEPT_ID <> 'Total'
  AND Questions = 'Likelihood of recommending'
  AND CAST(SUBSTRING(Received_Date, 1, CHARINDEX('-',Received_Date) -2) AS DATE) = '12/1/2023'
...
/*
  SELECT DISTINCT
   ...
   ,Responses
   ,CASE WHEN epic_department_id IS NULL THEN 'N' ELSE 'Y' END AS [In Mapping Epic_Dept_Groupers]
   ,ambulatory_flag
...
SELECT [Service]
  ...
  FROM #pg
  WHERE Questions = 'Likelihood of recommending'
  AND Start_Received_Date_dte = '12/1/2023'
  AND ambulatory_flag = 1
  --AND (ambulatory_flag = 1 or community_health_flag = 1)
) pg
...
SELECT
	tabrptg.epic_department_id,
    tabrptg.epic_department_name,
    tabrptg.LikelihoodRecommendPracticeResponse,
    pgf.pgf_responses
FROM
(
SELECT
