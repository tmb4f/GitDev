...
--ALTER PROCEDURE [ETL].[uspSrc_SvcLine_PatientProgression_TCHPlacement]
--    (
...
-------------------------------------------------------------------------------------------------------------------------
--INFO: 
--      INPUTS:	DS_HSDW_Prod.dbo.Dim_Date
--              DS_HSDM_App.Rptg.vwCuraspan_by_discharge
--				DS_HSDW_Prod.Rptg.vwDim_Patient
--				DS_HSDW_Prod.Rptg.vwFact_Pt_Enc_Hsp_Clrt
--				DS_HSDW_Prod.Rptg.vwFact_Pt_Enc_Atn_Prov_All
--				DS_HSDM_App.CMS.Curaspan_by_discharge_History
...
--         07/11/2019 -TMB - edited per peer review
--       
--************************************************************************************************************************
...
IF OBJECT_ID('tempdb..#navihealth ') IS NOT NULL
DROP TABLE #navihealth

SELECT DISTINCT

 --flags	
		--CAST('TCH_Placement' AS VARCHAR(50))				'event_type'
		CAST(event_type AS VARCHAR(50))						'event_type'
...
FROM DS_HSDW_Prod.dbo.Dim_Date dmdt
		LEFT OUTER JOIN  (SELECT	   hscdc.[PAT_ENC_CSN_ID]
									  ...
									  ,CASE 
									     WHEN hscdc.[Provider_Name] LIKE 'UVA Encompass%' THEN 'Encompass_Placement'
							             WHEN hscdc.[Provider_Name] LIKE 'UVA Transitional%' THEN 'TCH_Placement'
									     ELSE NULL
									   END AS event_type
						  ...
						  SELECT	   peh.[PAT_ENC_CSN_ID] AS PAT_ENC_CSN_ID
									  ...
									  ,atn.sk_Dim_Clrt_SERsrc
									  ,CASE 
									     WHEN cbd.[Provider Name] LIKE 'UVA Encompass%' THEN 'Encompass_Placement'
							             WHEN cbd.[Provider Name] LIKE 'UVA Transitional%' THEN 'TCH_Placement'
									     ELSE NULL
									   END AS event_type
...
SELECT	event_type,
        ...
        CMS_Number
FROM #navihealth
WHERE ((Placement_Provider_Name LIKE 'UVA Encompass%')
       OR (Placement_Provider_Name LIKE 'UVA Transitional%'))
	  AND event_count = 1
--ORDER BY  event_date
--ORDER BY  Final_Hospital_Status
--         ,event_date
--ORDER BY  Final_Hospital_Status
--         ,Provider_Name
--         ,event_date
ORDER BY  Placement_Provider_Name
         ,event_date
