...
INFO: 
	...
	This is filtered specifically to user activity in the Missing Reg Items workqueues.
		...
		OUTPUTS:
			Granularity at the workqueue item level, query pulls the items that have been entered into and\or released from the workqueue.
			...
			The event_date is the EXIT_DATE documented on a workqueue item; if NULL, it is set to the ENTRY_DATE value.
			The event_count value is set to 1 when the EXIT_DATE value is not NULL.
-----------------------------------------------------------------------------------------------------------------------
MODS: 	
...
CREATE CLUSTERED INDEX detail ON #detail (RN, DEPARTMENT_ID, ENTRY_DATE, EXIT_DATE, ENTRY_USER, EXIT_USER, ITEM_ID)
...
SELECT
 ...
 --   wq.Elapsed_Time,
	...
	wq.WQ_USER_NAME,
	wq.UVA_Computing_ID,
	wq.wd_Supervisory_Organization_ID,
	wq.wd_Supervisory_Organization_Description,
	wq.workday_supervisory_org_name,
	wq.workday_supervisory_org_id,
	wq.access_team_id,
	wq.access_team_name,
    wq.RULE_ID,
    wq.RULE_NAME
...
/* Fac/Org info */
,   'epic_department_id'			=	activity.DEPARTMENT_ID
...
,	'event_category'				=	CAST(activity.ITEM_STATUS AS VARCHAR(150)) 
...
,   mri.DEPARTMENT_ID
--,   mri.ITEM_STATUS
,   'Entry' AS ITEM_STATUS
,   CAST(mri.ENTRY_DATE AS DATE) AS event_date
--,   mri.EXIT_DATE
--,   mri.Elapsed_Time
,   mri.ENTRY_USER AS WQ_USER
--,   emp_entry.NAME AS WQ_USER_NAME
--,   WDemp_entry.UVA_Computing_ID AS Entry_UVA_Computing_ID
--,   WDemp_entry.wd_Supervisory_Organization_ID AS Entry_wd_Supervisory_Organization_ID
--,   WDemp_entry.wd_Supervisory_Organization_Description AS Entry_wd_Supervisory_Organization_Description
--,   so1.workday_supervisory_org_name AS Entry_workday_supervisory_org_name
--,   so1.workday_supervisory_org_id AS Entry_workday_supervisory_org_id
--,   at1.access_team_id AS Entry_access_team_id
--,   at1.access_team_name AS Entry_access_team_name
--,   mri.EXIT_USER
--,   emp_exit.NAME AS EXIT_NAME
--,   WDemp_exit.UVA_Computing_ID AS Exit_UVA_Computing_ID
--,   WDemp_exit.wd_Supervisory_Organization_ID AS Exit_wd_Supervisory_Organization_ID
--,   WDemp_exit.wd_Supervisory_Organization_Description AS Exit_wd_Supervisory_Organization_Description
--,   so2.workday_supervisory_org_name AS Exit_workday_supervisory_org_name
--,   so2.workday_supervisory_org_id AS Exit_workday_supervisory_org_id
--,   at2.access_team_id AS Exit_access_team_id
--,   at2.access_team_name AS Exit_access_team_name
--,   mri.USER_MATCH_YN
,   mri.RULE_ID
,   mri.RULE_NAME
--,   CONVERT(DATE, COALESCE(mri.EXIT_DATE,mri.ENTRY_DATE)) AS event_date -- Item Exit or Entry Date
...
--,   mri.ITEM_STATUS
,   'Exit' AS ITEM_STATUS
,   CAST(mri.EXIT_DATE AS DATE) AS event_date
--,   mri.EXIT_DATE
--,   mri.Elapsed_Time
,   mri.EXIT_USER AS WQ_USER
--,   emp_exit.NAME AS WQ_USER_NAME
--,   WDemp_entry.UVA_Computing_ID AS Entry_UVA_Computing_ID
--,   WDemp_entry.wd_Supervisory_Organization_ID AS Entry_wd_Supervisory_Organization_ID
--,   WDemp_entry.wd_Supervisory_Organization_Description AS Entry_wd_Supervisory_Organization_Description
--,   so1.workday_supervisory_org_name AS Entry_workday_supervisory_org_name
...
AND detail.EXIT_DATE IS NOT NULL
		...
		LEFT JOIN CLARITY..CLARITY_DEP	   DEP ON activity.DEPARTMENT_ID = DEP.DEPARTMENT_ID
		--INNER JOIN CLARITY..PATIENT		   pat ON mri.PAT_ID			   = pat.PAT_ID
		LEFT JOIN CLARITY_App.dbo.Dim_Date	dd	ON	CONVERT(DATE, activity.event_date) = dd.day_date
		...
		LEFT JOIN
					( --mdm history to replace vwRef_MDM_Location_Master_EpicSvc and vwRef_MDM_Location_Master
						...
						ON mdmdept.EPIC_DEPARTMENT_ID = activity.DEPARTMENT_ID	
		LEFT JOIN	CLARITY_App.Rptg.vwCLARITY_DEP	vwdep		ON	activity.DEPARTMENT_ID = vwdep.DEPARTMENT_ID			
		--LEFT JOIN	CLARITY_App.Rptg.vwDim_Clrt_Pt pt ON mri.PAT_ID = pt.Clrt_PAT_ID
		LEFT JOIN	CLARITY..CLARITY_EMP emp ON emp.USER_ID = activity.WQ_USER
		LEFT JOIN 
		(			 
