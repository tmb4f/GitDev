...
--WHERE (wquh.USER_ID = '62432' OR wqux.USER_ID = '62432')
--AND pe.EFFECTIVE_DEPT_ID = 10419014
)
...
CREATE CLUSTERED INDEX entry_index ON #entry (DEPARTMENT_ID, WQ_USER, event_date, RULE_ID)
...
CREATE CLUSTERED INDEX exit_index ON #exit (DEPARTMENT_ID, WQ_USER, event_date, RULE_ID)
...
--ORDER BY
--	COALESCE(entry.DEPARTMENT_ID,[exit].DEPARTMENT_ID),
--	COALESCE(entry.WQ_USER,[exit].WQ_USER),
--	COALESCE(entry.event_date,[exit].event_date),
--	COALESCE(entry.RULE_ID,[exit].RULE_ID)
)
...
--ORDER BY DEPARTMENT_ID, WQ_USER, event_date, RULE_ID
ORDER BY WQ_USER, event_date, RULE_ID, DEPARTMENT_ID
--CREATE CLUSTERED INDEX summary_index ON #summary (DEPARTMENT_ID, WQ_USER, event_date, RULE_ID)
...
SELECT WQ_USER, emp.NAME AS WQ_USER_NAME, emp.SYSTEM_LOGIN INTO #user_index FROM user_index_cte wq LEFT JOIN	CLARITY..CLARITY_EMP emp ON emp.USER_ID = wq.WQ_USER
ORDER BY emp.SYSTEM_LOGIN
CREATE CLUSTERED INDEX user_index ON #user_index (SYSTEM_LOGIN)
;
WITH user_cte AS
(
SELECT
...
FROM #user_index wq
		--LEFT JOIN	CLARITY..CLARITY_EMP emp ON emp.USER_ID = wq.WQ_USER
		LEFT JOIN 
		(			 
		...
		) WDemp ON wq.SYSTEM_LOGIN = UPPER(WDemp.UVA_Computing_ID) AND WDemp.rownum = 1
...
SELECT user_cte.WQ_USER, user_cte.WQ_USER_NAME, user_cte.wd_Supervisory_Organization_ID, user_cte.wd_Supervisory_Organization_Description, user_cte.access_team_id, user_cte.access_team_name INTO #user FROM user_cte
...
--DROP INDEX summary_index ON #summary;
--GO

--CREATE CLUSTERED INDEX summary_index ON #summary (WQ_USER, DEPARTMENT_ID, event_date, RULE_ID) WITH (DROP_EXISTING = ON);
...
--CREATE CLUSTERED INDEX summary2_index ON #summary2 (DEPARTMENT_ID, WQ_USER, event_date, RULE_ID);
/*
SELECT
	wq.event_date
,	wq.event_count
...
FROM #summary wq
		LEFT JOIN #user [user]	ON [user].WQ_USER = wq.WQ_USER
		LEFT JOIN CLARITY..CLARITY_DEP	   DEP ON wq.DEPARTMENT_ID = DEP.DEPARTMENT_ID
		LEFT JOIN CLARITY_App.dbo.Dim_Date	dd	ON	CONVERT(DATE, wq.event_date) = dd.day_date
		LEFT JOIN
					( --mdm history to replace vwRef_MDM_Location_Master_EpicSvc and vwRef_MDM_Location_Master
						SELECT DISTINCT
								hx.max_dt
