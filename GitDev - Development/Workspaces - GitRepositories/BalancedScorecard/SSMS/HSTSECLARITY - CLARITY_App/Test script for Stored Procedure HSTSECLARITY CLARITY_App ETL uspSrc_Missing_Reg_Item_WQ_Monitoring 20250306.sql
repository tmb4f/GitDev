...
SELECT
	...
	ccer.RULE_NAME,
    --EXTRACT_DATE,
	ROW_NUMBER() OVER(PARTITION BY wqi.ITEM_ID ORDER BY pwr.EXTRACT_DATE DESC) AS ltst_row
INTO #pwr
FROM #wqi wqi
--LEFT OUTER JOIN CLARITY..PAT_WQI_RULES pwr
--	ON pwr.ITEM_ID = wqi.ITEM_ID
INNER JOIN CLARITY..PAT_WQI_RULES pwr
	ON pwr.ITEM_ID = wqi.ITEM_ID
LEFT OUTER JOIN CLARITY..CL_CHRG_EDIT_RULE ccer
	ON ccer.RULE_ID = pwr.RULE_ID
ORDER BY
	wqi.ITEM_ID

CREATE UNIQUE CLUSTERED INDEX IX_pwr ON #pwr (ITEM_ID, LINE, RULE_ID)

--SELECT
--	wqi.WORKQUEUE_ID,
--    pwrs.LINE,
--    pwrs.RULE_ID,
--	zrk.NAME AS RULE_KIND_NAME
--INTO #pwrs
--FROM #wqi wqi
--LEFT OUTER JOIN CLARITY..PAT_WQ_RULES pwrs
--	ON pwrs.WORKQUEUE_ID = wqi.WORKQUEUE_ID
--LEFT OUTER JOIN CLARITY..ZC_RULE_KIND zrk
--	ON zrk.RULE_KIND_C = pwrs.RULE_KIND_C
--ORDER BY
--	wqi.WORKQUEUE_ID

--CREATE UNIQUE CLUSTERED INDEX IX_pwrs ON #pwrs (WORKQUEUE_ID, LINE, RULE_ID)
...
FROM	#wqi wqi
LEFT OUTER JOIN CLARITY..PAT_WQ_ITEMS p	ON p.ITEM_ID = wqi.ITEM_ID
--		INNER JOIN CLARITY..WORKQUEUE_INFO WI ON p.WORKQUEUE_ID = WI.WORKQUEUE_ID
		...
		INNER JOIN (
			...
			FROM(
				...
				FROM #wqi wqi
					INNER JOIN clarity..WQ_USR_HISTORY hx ON wqi.ITEM_ID = hx.WQ_ITM_ID
					...
					LEFT OUTER JOIN clarity..clarity_emp emp ON hx.USER_ID = emp.USER_ID
				WHERE 1=1
					AND hx.WQ_ACTIVITY_C =1 -- Entry
				)hitem
			--)hx_in ON wqi.ITEM_ID = hx_in.WQ_ITM_ID 
			)hx_in ON wqi.ITEM_ID = hx_in.ITEM_ID 
	INNER JOIN (
			SELECT
			   --hitem.WQ_ITM_ID
			   hitem.ITEM_ID
	...
	--)hx_out ON wqi.ITEM_ID = hx_out.WQ_ITM_ID 
	)hx_out ON wqi.ITEM_ID = hx_out.ITEM_ID 
 ...
 --               --EXTRACT_DATE,
	...
	--	) pwr		ON pwr.ITEM_ID = wqi.ITEM_ID AND pwr.ltst_row = 1
	--	LEFT OUTER JOIN CLARITY..CL_CHRG_EDIT_RULE ccer	ON ccer.RULE_ID = pwr.RULE_ID
		...
		(
			...
			FROM #pwr
			WHERE ltst_row = 1
		) pwr		ON pwr.ITEM_ID = wqi.ITEM_ID
		--LEFT JOIN
		--(
		--	SELECT
		--		WORKQUEUE_ID,
  ...
  --              RULE_KIND_NAME
		--	FROM #pwrs
		--) pwrs ON pwrs.WORKQUEUE_ID = wqi.WORKQUEUE_ID
		--					AND pwrs.RULE_ID = pwr.RULE_ID
