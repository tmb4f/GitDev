...
FROM
(
SELECT DISTINCT ca.EPIC_DEPARTMENT_ID, ca.sk_Ref_Clinical_Area_Map, ca.clinical_area_id, ca.clinical_area_name, ca.sk_Ref_Service_Map, COALESCE([service].service_id,9999) AS service_id, COALESCE([service].[service_name], 'Unmapped') AS [service_name], COALESCE([service].organization_id,9999) AS organization_id
FROM
(
SELECT DISTINCT mapping.EPIC_DEPARTMENT_ID, mapping.sk_Ref_Clinical_Area_Map, COALESCE(clinical_area.clinical_area_id,9999) AS clinical_area_id, COALESCE(clinical_area.clinical_area_name,'Unmapped') AS clinical_area_name, COALESCE(clinical_area.sk_Ref_Service_Map,9999) AS sk_Ref_Service_Map
FROM
(
SELECT DISTINCT mdm.EPIC_DEPARTMENT_ID, COALESCE(grouper.sk_Ref_Clinical_Area_Map,9999) AS sk_Ref_Clinical_Area_Map
FROM
(
SELECT
...
LEFT JOIN CLARITY_App.Stage.AmbOpt_OneTeam_Departments ot
ON ot.DEPARTMENT_ID = org.EPIC_DEPARTMENT_ID
