...
MODS: 
	...
	11/21/2024 - TB:	added level-of-care description
	12/19/2024 - TB:	added level-of-care description for previous (transfer) ADT event
	01/02/2025 - TB: updated logic to extract department attributes from MDM history
***********************************************************************************************************************/
...
SELECT
   ...
   ,transfers.level_of_care
   ,transfers.prv_level_of_care
  ...
  LEFT OUTER JOIN (SELECT DISTINCT
					 ...
					 ,zloc.NAME AS level_of_care
					 ,zloc2.NAME AS prv_level_of_care
...
---					INNER JOIN CLARITY.dbo.CLARITY_ADT adtprv ON adtprv.EVENT_ID = adt.PREV_EVENT_ID
					...
					LEFT OUTER JOIN CLARITY.dbo.ZC_LVL_OF_CARE zloc ON zloc.LEVEL_OF_CARE_C = pnd.LVL_OF_CARE_C
					LEFT OUTER JOIN CLARITY.dbo.ZC_LVL_OF_CARE zloc2 ON zloc2.LEVEL_OF_CARE_C = adtprv.PAT_LVL_OF_CARE_C
...
-- Service Lines by transfer location (for eventual use -- med ctr level will be attributed to Patient Progression service line in Tableau)
  ...
  LEFT OUTER JOIN #mdm orlocsvc ON orlocsvc.epic_department_id = transfers.origin_dept_id
  ...
  LEFT OUTER JOIN #mdm oclocsvc ON oclocsvc.epic_department_id = transfers.occupied_dept_id
