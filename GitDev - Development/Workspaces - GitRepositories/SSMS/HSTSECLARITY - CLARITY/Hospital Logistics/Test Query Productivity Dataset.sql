    ...
    (
        SELECT RECORD_ID
              ,CASE
               ...
               END       'CleanSts'
              ,INSTANT_TM
        FROM CLARITY..CL_BEV_EVENTS_ALL
    ) AS event
    PIVOT
    (
        MAX(INSTANT_TM)
...
AS (SELECT bev_cte.RECORD_ID
          ...
          ,DATENAME(dw, bev_cte.Dirty)                         'RequestDy'
          ,MONTH(bev_cte.Dirty)                                'RequestMonth'
          ,DATEDIFF(ss, bev_cte.Dirty, bev_cte.Assigned)       'GapRqstAssng'
          ,DATEDIFF(ss, bev_cte.Assigned, bev_cte.InProgress)  'GapAssInPgr'
          ,DATEDIFF(ss, bev_cte.InProgress, bev_cte.Completed) 'GapInpgrCmplt'
          ,DATEDIFF(ss, bev_cte.Dirty, bev_cte.InProgress)     'GapRqstInpgr'
          ,DATEDIFF(ss, bev_cte.Dirty, bev_cte.Completed)      'TAT'
          ,CASE
               WHEN DATEPART(hh, bev_cte.Dirty) < 7
               THEN '00:00-06:59'
               WHEN DATEPART(hh, bev_cte.Dirty) < 15
           ...
           END                                                 'HrGrp'
    FROM bev_cte
        INNER JOIN CLARITY..CL_BEV_ALL           bevall
            ON bevall.RECORD_ID = bev_cte.RECORD_ID
        LEFT OUTER JOIN hkr_cte
            ON hkr_cte.RECORD_ID = bev_cte.RECORD_ID
        INNER JOIN CLARITY..CLARITY_DEP          dept
            ON bevall.DEP_ID = dept.DEPARTMENT_ID
        INNER JOIN MDM_REV_LOC_ID                AS mloc
            ON mloc.REV_LOC_ID = dept.REV_LOC_ID
        INNER JOIN CLARITY..CLARITY_BED          bed
            ON bevall.BED_ID = bed.BED_ID
        LEFT OUTER JOIN CLARITY..ZC_PRIORITY_2   sts
            ON bevall.PRIORITY_C = sts.PRIORITY_2_C
        LEFT OUTER JOIN CLARITY..ZC_ESC_REASON   esc
            ON bevall.ESC_REASON_C = esc.ESC_REASON_C
        LEFT OUTER JOIN CLARITY..ZC_EVENT_SOURCE src
            ON bevall.EVENT_SOURCE_C = src.EVENT_SOURCE_C
        INNER JOIN CLARITY..CL_HKR               emp
            ON emp.RECORD_ID = hkr_cte.Assigned
    WHERE bevall.EVENT_TYPE_C = 0
          AND
          (
