...
--)
    AND (
	DEPARTMENT_ID = '10211004' -- F415 UNIV PHYS
	OR DEPARTMENT_ID = '10295006' -- CPSE CH CANC INF
	)
    --AND PROV_ID = '8'
    --AND CONTACT_DATE BETWEEN '2023-7-1' AND '2023-8-17'
    AND CONTACT_DATE BETWEEN '2024-7-1' AND '2024-8-31'
    AND APPT_STATUS_C = 2
;
...
SELECT --TOP 100
	...
	, ser.PROV_ID AS SER_PROV_ID
	, ser.PROV_NAME
	, zpt.NAME AS PROVIDER_TYPE_NAME
	, hsp.VISIT_PROV_ID
	, ser2.PROV_NAME AS Atn_Dr
...
FROM  ED_IEV_PAT_INFO       ievpat
	...
	LEFT JOIN CLARITY_SER ser ON ser.USER_ID = emp.USER_ID
	LEFT JOIN CLARITY_LOC loc ON loc.LOC_ID = ievevent.LOCATION_ID
	LEFT JOIN ZC_PROV_TYPE	zpt						ON zpt.PROV_TYPE_C = ser.PROVIDER_TYPE_C

	LEFT JOIN PAT_ENC hsp		ON hsp.PAT_ENC_CSN_ID = ievpat.PAT_ENC_CSN_ID
	LEFT JOIN CLARITY_SER ser2 ON ser2.PROV_ID = hsp.VISIT_PROV_ID
...
AND ievevent.EVENT_TYPE IS NOT NULL
AND ievevent.EVENT_DISPLAY_NAME IS NOT NULL
)
...
SELECT
	...
	,   cte.SER_PROV_ID
	,   cte.PROV_NAME
	,   cte.PROVIDER_TYPE_NAME
	,   cte.VISIT_PROV_ID
	,   cte.Atn_Dr
    ,   cte.EVT_TMPLT_CLASS_C_pat
    ,   EVT_TMPLT_CLASS_C_name_pat
FROM cte
WHERE cte.PROVIDER_TYPE_NAME IS NOT NULL
--ORDER BY
--        PAT_ID
--    ,   PAT_ENC_CSN_ID
--    ,   EVENT_TIME_iev
