...
SELECT
      ...
      --,[PAT_ENC_DATE_REAL]
	  ,nte.Amb_Note
	  ,nte.sk_Dim_Clrt_Note_Typ_IP
	  ,ntetyp.Note_Typ_IP_Nme
      --,ntetxt.Note_Txt
      --,[Load_Dtm]
  INTO #neopt
  FROM @neodata neodata
  INNER JOIN DS_HSDW_Prod.Rptg.vwFact_Pt_Enc_Clrt enc
	ON neodata.sk_Dim_Clrt_Pt  = enc.sk_Dim_Clrt_Pt
  INNER JOIN [DS_HSDW_Prod].[Rptg].[vwFact_Pt_Enc_Clrt_Note] nte
	ON enc.sk_Fact_Pt_Enc_Clrt = nte.sk_Fact_Pt_Enc_Clrt
  INNER JOIN DS_HSDW_Prod.Rptg.vwFact_Pt_Enc_Clrt_Note_Txt ntetxt
	ON nte.sk_Fact_Pt_Enc_Clrt_Note = ntetxt.sk_Fact_Pt_Enc_Clrt_Note
  LEFT OUTER JOIN DS_HSDW_Prod.Rptg.vwDim_Clrt_Note_Typ_IP ntetyp
	ON ntetyp.sk_Dim_Clrt_Note_Typ_IP = nte.sk_Dim_Clrt_Note_Typ_IP
  --WHERE Appt_Note LIKE 'cerebral palsy'
  WHERE 1 = 1
  --AND enc.sk_Dim_Clrt_Pt	 = 480
  --AND neodata.sk_Dim_Pt	 = 2082221
  --AND MedRecNum	= '2338112'
  --AND nte.sk_Fact_Pt_Enc_Clrt = 67589760
  AND ntetxt.Note_Txt LIKE 'cerebral palsy'

  SELECT
	*
  FROM #neopt
  ORDER BY sk_Dim_Clrt_Pt, sk_Fact_Pt_Enc_Clrt--, Note_LINE
/*
 --   SELECT
	--ntes.sk_Dim_Clrt_Pt,
