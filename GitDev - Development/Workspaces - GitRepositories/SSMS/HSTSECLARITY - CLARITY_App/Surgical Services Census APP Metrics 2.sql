...
CREATE TABLE #AD_Events (PAT_ID VARCHAR(18), PAT_ENC_CSN_ID  numeric, Date_Time  datetime, SCHED_START_TIME DATETIME, bill_num  varchar(50)
	, Event varchar(254), OR_Pt_Class varchar(254), Unit varchar(254), Room varchar(254), id varchar(254), Current_Service varchar(254), Service varchar(254)
...
ON ect.PAT_ID = adt.PAT_ID
AND ect.PAT_ENC_CSN_ID = adt.PAT_ENC_CSN_ID
...
WHERE orlog.seq =  1
	...
	SELECT DISTINCT 
		...
		,hsp.HSP_ACCOUNT_ID
		--,eta.TEAM_AUDIT_ID
		,pats.CARE_TEAMS_ID AS TEAM_AUDIT_ID
		--,ptri.RECORD_NAME
		,pats.RECORD_NAME
		--,eta.TEAM_AUDIT_INSTANT
		,orlog.OR_Pt_Class
	...
	ON pats.PAT_ENC_CSN_ID = hsp.PAT_ENC_CSN_ID
--	LEFT OUTER JOIN
--	(
--	SELECT * FROM 
--							(SELECT 
...
--ON flshtpvt.INPATIENT_DATA_ID = hsp.INPATIENT_DATA_ID
	...
	LEFT OUTER JOIN #AD_Events orlog
		...
		ON orlog.PAT_ID = pats.PAT_ID
		AND orlog.PAT_ENC_CSN_ID = pats.PAT_ENC_CSN_ID
...
*/
    ...
    ORDER BY pats.PAT_ID, pats.PAT_ENC_CSN_ID
...
*/
	...
	----AND eta.TEAM_AUDIT_INSTANT <= adt.EFFECTIVE_TIME
	INNER JOIN #eta eta
	ON adt.PAT_ID = eta.PAT_ID
	AND adt.PAT_ENC_CSN_ID = eta.PAT_ENC_CSN_ID
	) adt
