...
AS (SELECT ClnSts.*
    ...
    (
        MAX(INSTANT_TM)
        FOR CleanSts IN ([Dirty], [Assigned], [InProgress], [Completed])
    ) AS ClnSts
	INNER JOIN CLARITY..HL_REQ_INFO cba
	ON ClnSts.RECORD_ID = cba.HLR_ID
	/*
	HLR_TYPE_C: 1 = Request, 2 = Group,  3 = Job
	PRIMARY_LINKED_EVENT_C: ZC_HL_REQ_LINKED_EVENT
	*/
	WHERE
...
, dept.DEPARTMENT_ID
    ...
    FROM bev_cte
		...
		SELECT
			...
			ROW_NUMBER() OVER(PARTITION BY evnts.HLR_ID ORDER BY LINE) AS seq2
...
ORDER BY HOSPITAL_CODE, evs_cte.DEPARTMENT_NAME, evs_cte.REQ_BED_ID, evs_cte.TimeDirty
--*/