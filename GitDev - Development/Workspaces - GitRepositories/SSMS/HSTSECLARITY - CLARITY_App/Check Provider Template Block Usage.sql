...
--SET @HierarchyValues = '10' --	Gastroenterology
SET @HierarchyValues = '8' -- UPG -- CPG PRIMARY CARE -- Test
...
SELECT
	...
	SUM(CASE WHEN appt_slot_number = 0 AND UNAVAILABLE_RSN_C IS NULL THEN regular_openings ELSE 0 END) AS regular_openings_available
FROM #avail_slot
GROUP BY
	department_id,
	prov_id,
...
SELECT DISTINCT
	...
	MAX(sum2.org_avail_block_names) AS org_avail_block_names
...
SELECT
	...
	GROUP BY
		...
		innerTable.prov_id,
		innerTable.slot_begin_time,
		innerTable.BLOCK_NAME
	FOR XML PATH('')
	),1,1,'') AS org_avail_block_names
FROM #avail_block p
GROUP BY
	department_id,
	prov_id,
...
ON sum2.department_id = blk.department_id
AND sum2.prov_id = blk.prov_id
AND sum2.slot_begin_time = blk.slot_begin_time
GROUP BY
	blk.department_id,
	blk.prov_id,
	blk.slot_begin_time
...
SELECT
 ...
 --   CASE WHEN summary.block_name IS NULL AND booked_by_block.booked IS NULL THEN NULL -- ******************************** --
 --              WHEN summary.block_name IS NULL AND booked_by_block.booked IS NOT NULL THEN 'Unknown'
	--		   WHEN summary.block_name IS NOT NULL AND booked_by_block.booked IS NULL THEN summary.block_name
	--		   ELSE summary.block_name
	--END AS block_name,
...
--SELECT
--	*
--FROM #booked_available_by_block summary
ORDER BY
	summary.DEPARTMENT_ID,
    summary.PROV_ID,
    summary.SLOT_BEGIN_TIME,
...
INTO #availability
FROM #booked_available_summary s1
LEFT JOIN #org_avail_blocks_summary s2
	ON s2.DEPARTMENT_ID = s1.DEPARTMENT_ID
	AND s2.PROV_ID = s1.PROV_ID
	AND s2.SLOT_BEGIN_TIME = s1.SLOT_BEGIN_TIME
LEFT JOIN [CLARITY_App].[Mapping].[Epic_Dept_Groupers] g ON s1.DEPARTMENT_ID = g.epic_department_id
...
SELECT
    ...
    Slot_Begin_Time,
    Slot_Begin_Date,
    Total_Regular_Openings_Available,
