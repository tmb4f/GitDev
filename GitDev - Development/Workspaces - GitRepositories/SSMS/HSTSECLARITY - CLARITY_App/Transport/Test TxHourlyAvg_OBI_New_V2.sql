...
txp_nxt AS 
	...
	SELECT
		...
		,CLARITY.EPIC_UTIL.EFN_UTC_TO_LOCAL(hri.REQ_START_UTC_DTTM)        AS [txprtNxt] -- Start time
	...
	WHERE 1=1
		...
		AND CLARITY.EPIC_UTIL.EFN_UTC_TO_LOCAL(hri.REQ_START_UTC_DTTM) BETWEEN @startdate AND @enddate
	)
...
,txphkr AS
	...
	FROM
		...
		SELECT 
			 ...
			 CAST(CLARITY.EPIC_UTIL.EFN_UTC_TO_LOCAL(hse.SHIFT_EVENT_INST_UTC_DTTM) AS DATE) AS [contact_date]
			--,hse.SHIFT_EVENT_INST_UTC_DTTM
			,CLARITY.EPIC_UTIL.EFN_UTC_TO_LOCAL(hse.SHIFT_EVENT_INST_UTC_DTTM) AS SHIFT_EVENT_INST_LOCAL_DTTM
		...
		MAX(SHIFT_EVENT_INST_LOCAL_DTTM)
...
--select * from txp_cte where ASSIGNED_TECH_ID = 1646 ORDER BY txp_cte.HLR_ID, txp_cte.ASSIGNED_TECH_ID 
