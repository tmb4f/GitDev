...
DECLARE @startdate DATETIME = '4/01/2024';
...
IF OBJECT_ID('tempdb..#MDM_REV_LOC_ID ') IS NOT NULL
DROP TABLE #MDM_REV_LOC_ID

IF OBJECT_ID('tempdb..#transport ') IS NOT NULL
DROP TABLE #transport

IF OBJECT_ID('tempdb..#txp ') IS NOT NULL
DROP TABLE #txp

IF OBJECT_ID('tempdb..#month1 ') IS NOT NULL
DROP TABLE #month1

IF OBJECT_ID('tempdb..#month2 ') IS NOT NULL
DROP TABLE #month2
...
SELECT DISTINCT
		...
		,dte.fmonth_num
		,dte.month_name	
		,dte.day_of_week_num AS DOWn
		,dte.day_of_week AS DOW
		,DAY(dte.day_date) AS DayNum
		,dte.day_date AS ActivationDate
...
GROUP BY fmonth_num, txp.ActivationDate, mloc.DE_HOSPITAL_CODE

SELECT
	*
FROM #month1
ORDER BY MnthNum, ActivationDate, Hospital_Code
*/
/*
SELECT 
		 --loc_id
...
GROUP BY fmonth_num, txp.DOWn, mloc.DE_HOSPITAL_CODE
...
SELECT
	...
	m1.DOW,
    m1.Hospital_Code,
    m1.Total_Requests AS March_Transport_Requests,
    m2.Month,
    --m2.MnthNum,
    --m2.DayNum,
	m2.DOW,
    --m2.Hospital_Code,
    m2.Total_Requests AS April_Transport_Requests
FROM #month1 m1
LEFT OUTER JOIN #month2 m2
--ON m2.DayNum = m1.DayNum
ON m2.DOWn = m1.DOWn
ORDER BY
	m1.Month, m1.DOWn
*/
