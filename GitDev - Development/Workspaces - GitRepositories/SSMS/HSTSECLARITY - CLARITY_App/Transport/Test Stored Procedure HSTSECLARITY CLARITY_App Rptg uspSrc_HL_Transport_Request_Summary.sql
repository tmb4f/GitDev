...
--FROM #transport
--WHERE REQ_TASK_SUBTYPE_C = 1 -- Patient Transport
--ORDER BY
--	HLR_ID,
--	EVENT_LOCAL_DTTM
...
(
SELECT txp.HLR_ID, txp.EVENT_LOCAL_DTTM, txp.REQ_HOSP_LOC_ID
FROM #transport txp
WHERE
...
(
--SELECT *
SELECT txp.HLR_ID, txp.EVENT_LOCAL_DTTM
FROM #transport txp
...
(
SELECT DISTINCT
		txp_src.HLR_ID, txp_src.REQ_HOSP_LOC_ID
		...
		,CASE WHEN txprqst.REQ_TASK_SUBTYPE_C IN ('1', '99')
				THEN	1
				ELSE	0
				END																												'Pt_Txp_Rqst'
		,CASE WHEN txprqst.REQ_TASK_SUBTYPE_C IN ('2', '3')
				THEN	1
				ELSE	0
				END																												'Non-Pt_Txp_Rqst'
		...
		,CASE WHEN (txprqst.REQ_TASK_SUBTYPE_C IN ('2', '3') AND txp_dlyd.HLR_ID IS NOT NULL)
		...
		,CASE WHEN DATEDIFF(MINUTE,txp_pnd.EVENT_LOCAL_DTTM,txp_cmplt.EVENT_LOCAL_DTTM)<=60
				...
				END																												'compliance_60'
		,CASE WHEN (DATEDIFF(MINUTE,txp_pnd.EVENT_LOCAL_DTTM,txp_cmplt.EVENT_LOCAL_DTTM)<=30
						AND txprqst.REQ_TASK_SUBTYPE_C IN ('1', '99'))
				THEN 1
				ELSE 0
		...
		,CASE WHEN (DATEDIFF(MINUTE,txp_pnd.EVENT_LOCAL_DTTM,txp_cmplt.EVENT_LOCAL_DTTM)<=30
						AND txprqst.REQ_TASK_SUBTYPE_C IN ('2', '3'))
				THEN 1
				ELSE 0
...
FROM txp_src
LEFT OUTER JOIN txp_asgn							ON txp_src.HLR_ID=txp_asgn.HLR_ID
LEFT OUTER JOIN txp_inpgr							ON txp_src.HLR_ID=txp_inpgr.HLR_ID
LEFT OUTER JOIN txp_pnd								ON txp_src.HLR_ID=txp_pnd.HLR_ID
LEFT OUTER JOIN txp_cncl							ON txp_src.HLR_ID=txp_cncl.HLR_ID
LEFT OUTER JOIN txp_pstpn							ON txp_src.HLR_ID=txp_pstpn.HLR_ID
LEFT OUTER JOIN txp_dlyd							ON txp_src.HLR_ID=txp_dlyd.HLR_ID
LEFT OUTER JOIN txp_acknw							ON txp_src.HLR_ID=txp_acknw.HLR_ID
LEFT OUTER JOIN txp_cmplt							ON txp_src.HLR_ID=txp_cmplt.HLR_ID
--INNER JOIN CLARITY.dbo.TXPORT_REQ_INFO txprqst			ON txprqst.TRANSPORT_ID=txp_src.HLR_ID
INNER JOIN #transport txprqst			ON txprqst.HLR_ID=txp_src.HLR_ID
