...
You need to pull all three sectors 3100000108, 3100000086, 3100000113  to pull all hospitals (for transport requests). 

3100000108 for Culpeper
3100000086  For UVA
3100000113 for prince William

Other sectors 3100000057, 3100000000, 3100000072 are EVS and should not be pulled in for transport reporting. 
*/
...
SELECT DISTINCT
		...
		,CASE WHEN (txprqst.REQ_TASK_SUBTYPE_C = '99'
				...
				ELSE	0
				END																												'Non-Pt_dlyd_Txp_Rqst'
		,CAST(txp_src.EVENT_LOCAL_DTTM AS DATE) AS EVENT_LOCAL_DT
		,dte.fmonth_num
		,dte.month_name	
		,txp_pnd.EVENT_LOCAL_DTTM																						'Request_Pend_Time'
		,txp_asgn.EVENT_LOCAL_DTTM																						'Rqst_Assigned_Time'
...
GROUP BY fmonth_num, txp.EVENT_LOCAL_DT

ORDER BY txp.fmonth_num, txp.EVENT_LOCAL_DT
*/
GO
