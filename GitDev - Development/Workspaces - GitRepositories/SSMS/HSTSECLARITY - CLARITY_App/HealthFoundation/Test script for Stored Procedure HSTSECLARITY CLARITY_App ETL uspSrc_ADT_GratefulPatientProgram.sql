...
SELECT DISTINCT  --without distinct, results multiple rows becasue diagnosis code in multiple line
	...
	,zcfc.NAME AS ACCT_FIN_CLASS_NAME
	,acc.ACCT_SLFPYST_HA_C
	,zash.NAME AS ACCT_SLFPYST_HA_NAME
	,fpl.FPL_STATUS_CODE_C
	,zfsc.NAME AS FPL_STATUS_CODE_NAME
	,accst.ACCOUNT_STATUS_C
	,zas.NAME AS ACCOUNT_STATUS_NAME
...
FROM CLARITY.dbo.CLARITY_SER ser
			...
			LEFT OUTER JOIN CLARITY.dbo.ZC_CUR_FIN_CLASS zcfc	ON zcfc.CUR_FIN_CLASS = acc.ACCT_FIN_CLASS_C
			LEFT OUTER JOIN CLARITY.dbo.ZC_ACCT_SLFPYST_HA zash ON zash.ACCT_SLFPYST_HA_C = acc.ACCT_SLFPYST_HA_C
			LEFT OUTER JOIN CLARITY.dbo.ZC_FPL_STATUS_CODE zfsc ON zfsc.FPL_STATUS_CODE_C = fpl.FPL_STATUS_CODE_C
			LEFT OUTER JOIN CLARITY.dbo.ZC_ACCOUNT_STATUS zas ON zas.ACCOUNT_STATUS_C = accst.ACCOUNT_STATUS_C
/*
ACCT_FIN_CLASS_C	ACCT_FIN_CLASS_NAME
1	Commercial
105	Medicare Replacement
12	Other
2	Medicare
205	Medicaid Managed Care
210	Medicaid Out of State
3	Medicaid
...
ACCT_SLFPYST_HA_C	ACCT_SLFPYST_HA_NAME
1	Full Self-Pay Due
3	Payment Plan
5	Bad Debt
6	Prorated

FPL_STATUS_CODE_C	FPL_STATUS_CODE_NAME
4	Denied for Charity
10	1S: 100% Financial Assistance Application Needed
12	2S: 95% Financial Assistance Application Needed
14	3S: 80% Financial Assistance Application Needed
16	4S: 55% Financial Assistance Application Needed
18	5S: 30% Financial Assistance Application Needed
19	9S: 60% Charity Application Needed
20	7S: Financial and Charity Assistance Over Scale
21	10S: 50% Charity Application Needed
22	1P: 100% Financial Assistance Pending
24	2P: 95% Financial Assistance Pending
26	3P: 80% Financial Assistance Pending
28	4P: 55% Financial Assistance Pending
30	5P: 30% Financial Assistance Pending
32	7P: Financial Assistance Over Scale
34	1V: 100% Financial Assistance Verified
36	2V: 95% Financial Assistance Verified
38	3V: 80% Financial Assistance Verified
40	4V: 55% Financial Assistance Verified
42	5V: 30% Financial Assistance Verified
44	7V: Financial Assistance Over Scale
46	8V: 100% Charity Verified
48	9V: 60% Charity Verified
50	10V: 50% Charity Verified
52	8P: 100% Charity Pending
54	9P: 60% Charity Pending
56	10P: 50% Charity Pending

ACCOUNT_STATUS_C	ACCOUNT_STATUS_NAME
1	Returned Mail
2	Payment Plan
3	Collections
4	Contested Accounts
100	No Personal Checks
101	Refer to Financial Services
102	Bankruptcy
103	NSF Check
104	Potential Duplicate
105	Legal
106	Deceased
107	Emancipated Minor
108	Employee
109	Possible Identity Theft
110	Financial Counselor Confirmed Self-Pay
111	Dependent Adult
117	Transplant Client
118	Pre-Collect
119	No Address
120	International patient
121	Client Rvw Not Required
122	Refund by Check
123	CO Exclusion
124	CO 1
125	CO 2
...
129	FA Special
130	FA Special LP
131	FA Special Pending
132	HPE Denied NQ
133	PFB Do Not Call List
140	Guarantor Risk Mgmt Hold - All
141	Guarantor Issue Do Not Bill
145	Guarantor Risk Mgmt Hold - Statement
147	PFB Director Hold
148	PFB OC - Duplicate Call Risk
151	PFB Director Hold - All Statements and Ins Billing 
*/
...
SELECT DISTINCT
	...
	,ACCT_FIN_CLASS_NAME
FROM #GPP
ORDER BY ACCT_FIN_CLASS_C

SELECT DISTINCT
	ACCT_SLFPYST_HA_C
	,ACCT_SLFPYST_HA_NAME
FROM #GPP
ORDER BY ACCT_SLFPYST_HA_C

SELECT DISTINCT
	FPL_STATUS_CODE_C
	,FPL_STATUS_CODE_NAME
FROM #GPP
ORDER BY FPL_STATUS_CODE_C

SELECT DISTINCT
ACCOUNT_STATUS_C
	,ACCOUNT_STATUS_NAME
FROM #GPP
ORDER BY ACCOUNT_STATUS_C
GO
