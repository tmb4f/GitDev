...
--SELECT CAST(CAST(20230616 AS VARCHAR) AS DATE);

--exec [ETL].[uspSrc_DAR_Opth_IntraVitrealInj_Inf_Rate]

--ALTER PROCEDURE [ETL].[uspSrc_DAR_Opth_IntraVitrealInj_Inf_Rate]
----ALTER PROCEDURE [ETL].[uspSrc_DAR_Opth_IntravitrealInj_Inf_Rate]
--AS
/**********************************************************************************************************************
WHAT: DAR Opthamology Intravitreal Injection for macular degeneration Infection Rate
WHO : Stephen E. Poff (guf2kd)
WHEN: 12/12/2024
WHY : User Story # 52651
-----------------------------------------------------------------------------------------------------------------------
INFO: 
---DAR FY 25
---Goal: <= 0.05% : Infection rate after intravitreal injection for macular degeneration 

      INPUTS:
             *A list of all tables and a brief description of their purpose in your stored procedure. 

    CDW_App.Rptg.vwRef_MDM_Location_Master_History
    CDW_App.Rptg.vwRef_MDM_Location_Master_Hospital_Group_ALL

    cdw.FullAccess.ProcedureDim
    cdw.fullaccess.DiagnosisEventFact  
    cdw.fullaccess.DiagnosisTerminologyDim 
    cdw.fullaccess.PatientDim 
    cdw.FullAccess.DiagnosisEventSourceBridge
    cdw.fullaccess.DepartmentDim 
    cdw.FullAccess.SurgicalProcedureEventFact         
    cdw.FullAccess.SurgicalProcedureCodeBridge
    cdw.FullAccess.SurgicalCaseFact        
    cdw.FullAccess.DurationDim                
    cdw.FullAccess.SourceDim   
    ...
    cdw.FullAccess.CodedProcedureFact               
    cdw.FullAccess.ProcedureTerminologyDim 
    cdw.FullAccess.ProcedureEventFact                  
    cdw.FullAccess.ProcedureDim              
    cdw.FullAccess.ProcedureMappingDim        
    cdw.FullAccess.TerminologyConceptDim      
...
--------------------------------------------------------------------------------------------------------------------------
MODS:	02/27/2025 DM2NB Add dates for date fill; add column names instead of using asterisk; comment out DROP TABLE statements
