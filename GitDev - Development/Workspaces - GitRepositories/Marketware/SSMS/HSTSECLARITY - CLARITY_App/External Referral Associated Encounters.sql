...
IF OBJECT_ID('tempdb..#extrfls ') IS NOT NULL
DROP TABLE #extrfls
...
SELECT DISTINCT
   ...
   REFERRAL_SOURCE.REF_PROVIDER_ID AS REFERRED_FROM_PROV_ID,
   --REFERRING_SER.PROV_NAME AS provider_name,
   REFERRING_SER.PROV_NAME AS REFERRED_FROM_PROV_NAME,
   RFL.ENTRY_DATE,
   RFL.EXP_DATE,
...
ORDER BY
  ...
  --CREATE CLUSTERED INDEX IX_extrfls ON #extrfls (person_id, REFERRAL_ID)
  CREATE CLUSTERED INDEX IX_extrfls ON #extrfls (REFERRAL_ID)
...
SELECT
    ...
    person_id,
    person_name,
    REFERRED_FROM_PROV_ID,
    REFERRED_FROM_PROV_NAME,
    ENTRY_DATE,
    EXP_DATE,
    REFERRAL_CLASS,
    REFERRAL_TYPE,
...
SELECT
	...
	RFL_ENC.ENC_TYPE_C,
	zdet.NAME AS ENC_TYPE_NAME
    FROM CLARITY.dbo.PAT_ENC RFL_ENC
	LEFT OUTER JOIN CLARITY.dbo.ZC_HOSP_ADMSN_TYPE zhat ON zhat.HOSP_ADMSN_TYPE_C = RFL_ENC.HOSP_ADMSN_TYPE_C
	LEFT OUTER JOIN CLARITY.dbo.ZC_DISP_ENC_TYPE zdet ON zdet.DISP_ENC_TYPE_C = RFL_ENC.ENC_TYPE_C
    --GROUP BY RFL.REFERRAL_ID
	) RFL_APPTS
	...
	ON extrfls.REFERRAL_ID = RFL_APPTS.RFL_ID
--ORDER BY
--	person_id,
--	REFERRAL_ID,
--	APPT_TIME,
...
ORDER BY
  ...
  --CREATE CLUSTERED INDEX IX_rflencs ON #rflencs (PAT_ENC_CSN_ID, REFERRAL_ID)
  CREATE CLUSTERED INDEX IX_rflencs ON #rflencs (RFL_ID, PAT_ENC_CSN_ID)
