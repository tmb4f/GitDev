...
WHEN: Daily
WHY :	Detail updates for completed outpatient and HOV (Hospital Outpatient Visit) encounters with "Person" resources
AUTHOR: Tom Burgan
SPEC:	
--------------------------------------------------------------------------------------------
INPUTS: 	        
...
MODS: 
		**		09/19/2024  -Tom B.  Create stored procedure
				...
				12/01/2024	-Tom B.  Add @LOC_TO_HOSP mapping table
				12/05/2024	-Tom B.  Use external mapping tables (Rptg.UA_GPP_Loc_to_Hosp, Rptg.UA_GPP_Guar_Rltn_to_Pt, Rptg.UA_GPP_Department_Mapping);
													Set value of discharge time, dependent on appointment status; Exclude appointments without an appointment status
...
-- =============================================
--EXEC [ETL].[uspSrc_UA_GratefulPatientProgram_OP_HOV]

--ALTER   PROCEDURE [ETL].[uspSrc_UA_GratefulPatientProgram_OP_HOV] 
...
SELECT DISTINCT  --without distinct, results multiple rows becasue diagnosis code in multiple line
	...
	,'Out-Patient'																									AS	'ucinn_ascendv2__Appointment_Type__c'
	...
	,hsp.HOSP_DISCHRG_TIME																			AS	'ucinn_ascendv2__Discharge_Date_Time__c'
...
LEFT OUTER JOIN CLARITY.dbo.CLARITY_SER ser				ON ser.PROV_ID = hsp.VISIT_PROV_ID 
			...
			LEFT OUTER JOIN CLARITY_App.Rptg.UA_GPP_Loc_to_Hosp loc		ON loc.LOC_ID = hsp.PRIMARY_LOC_ID
...
AND (
			(
				((hsp.APPT_STATUS_C IS NOT NULL AND hsp.APPT_STATUS_C IN ('1','2','3','4','5','6','102')) --ONLY SCHEDULED/COMPLETED/CANCELED/NO SHOW/LEFT WITHOUT SEEN/ARRIVED/WALK-IN STATUS
...
AND appt.APPT_STATUS_NAME IS NOT NULL
),
...
(
SELECT DISTINCT
	 --RIGHT('000000000000'+CAST(a.OR_LINK_CSN AS VARCHAR(12)),12)		AS 	'ucinn_ascendv2__External_System_ID__c'
	 RIGHT('000000000000'+CAST(a.PAT_ENC_CSN_ID AS VARCHAR(12)),12)		AS 	'ucinn_ascendv2__External_System_ID__c'
	,a.OR_LINK_CSN
	,clpt.sk_Dim_Pt
	,CASE WHEN clpt.sk_Dim_Pt <> -1 THEN RIGHT('0000000'+CAST(clpt.sk_Dim_Pt AS VARCHAR(8)),8) ELSE REPLACE(TRANSLATE(TRIM(pat.PAT_NAME),', ','__'	),'.','') END AS 	'ESI_3'
	,RIGHT('0000000000'+CAST(gacc.ACCOUNT_ID AS VARCHAR(10)),10)		AS	'ucinn_ascendv2__Guarantor_ID__c'
	,surg_ser.PROV_ID																								AS	'ucinn_ascendv2__Physician_ID__c'
	,CASE WHEN rship.GUAR_REL_TO_PAT_C = 15 THEN 'Self' ELSE ascrel.ASCEND_VALUE END 	AS	'ucinn_ascendv2__Guarantor_Relationship__c'
	,'In-Patient'																											AS	'ucinn_ascendv2__Appointment_Type__c'
	,CASE WHEN oloc.LOC_ID IS NULL THEN 'UVA Hospital' ELSE hloc.ASCEND_VALUE END AS 	'ucinn_ascendv2__Appointment_Location__c'
	--,ascdep.ASCEND_VALUE																				AS	'ucinn_ascendv2__Department_Formula__c'
	,loc.LOC_NAME																									AS	'ucinn_ascendv2__Department_Formula__c'
	,zos.NAME																											AS	'ucinn_ascendv2__Appointment_Status__c'
	,'Health System'																									AS	'ucinn_ascendv2__Data_Source__c'
	,oc.CASE_BEGIN_INSTANT																				AS	'ucinn_ascendv2__Admit_Date_Time__c'
	,CASE WHEN zos.NAME = 'Completed' THEN oc.CASE_END_INSTANT ELSE NULL END AS	'ucinn_ascendv2__Discharge_Date_Time__c'
	,har.ACCT_FIN_CLASS_C
	,har.ACCT_SLFPYST_HA_C
	,fpl.FPL_STATUS_CODE_C
	,fpl.FPL_EFF_DATE
	,accst.ACCOUNT_STATUS_C
	,pat.BIRTH_DATE
...
WHERE csaseq.updseq = 1
  ...
  ON x.OR_CASE_ID = oc.OR_CASE_ID
	...
	LEFT OUTER JOIN CLARITY.dbo.	PAT_ENC_HSP	hsp
		ON a.OR_LINK_CSN = hsp.PAT_ENC_CSN_ID
	LEFT OUTER JOIN CLARITY.dbo.HSP_ACCOUNT har
		ON har.HSP_ACCOUNT_ID = hsp.HSP_ACCOUNT_ID		
	LEFT OUTER JOIN CLARITY.dbo.ACCOUNT gacc
		ON har.GUARANTOR_ID = gacc.ACCOUNT_ID
	LEFT OUTER JOIN CLARITY.dbo.ACCT_GUAR_PAT_INFO rship
		ON rship.ACCOUNT_ID = gacc.ACCOUNT_ID
	LEFT OUTER JOIN CLARITY_App.Rptg.UA_GPP_Guar_Rltn_to_Pt ascrel
		ON ascrel.GUAR_REL_TO_PAT_C = rship.GUAR_REL_TO_PAT_C
	LEFT OUTER JOIN CLARITY.dbo.ACCOUNT_FPL_INFO fpl
		ON hsp.HSP_ACCOUNT_ID = fpl.ACCOUNT_ID
	LEFT OUTER JOIN CLARITY.dbo.ACCOUNT_STATUS	accst
		ON hsp.HSP_ACCOUNT_ID=accst.ACCOUNT_ID 
	LEFT OUTER JOIN CLARITY.dbo.HSP_ACCT_DX_LIST dx
		ON hsp.HSP_ACCOUNT_ID=dx.HSP_ACCOUNT_ID
																--AND dx.line=1
	LEFT OUTER JOIN CLARITY.dbo.CLARITY_EDG edg	
 ...
 --       ON op.OR_PROC_ID = ocap.OR_PROC_ID
	...
	LEFT OUTER JOIN CLARITY_App.Rptg.UA_GPP_Loc_to_Hosp hloc
		ON hloc.LOC_ID = oc.LOC_ID
...
SELECT
	*
FROM ORP oc
ORDER BY oc.SCHEDULED_SURGERY_DATE, PAT_MRN_ID
/*
SELECT
    ...
    CASE WHEN ucinn_ascendv2__Discharge_Date_Time__c IS NOT NULL
				THEN ucinn_ascendv2__Discharge_Date_Time__c
			   WHEN enc.APPT_STATUS_C IN ('1','2','3','4','5','6','102') -- Scheduled, Completed, Canceled, No Show, Left without seen, Arrived, Walk-in
				THEN ucinn_ascendv2__Admit_Date_Time__c
			   ELSE NULL
	END																																	AS ucinn_ascendv2__Discharge_Date_Time__c,
    enc.Load_Dtm
FROM OPHOV enc
WHERE 1 = 1
		AND (
