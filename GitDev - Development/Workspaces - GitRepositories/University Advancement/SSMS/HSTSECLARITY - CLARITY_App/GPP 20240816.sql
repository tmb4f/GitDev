...
SELECT DISTINCT  --without distinct, results multiple rows becasue diagnosis code in multiple line
	...
	,CAST(pt.PAT_MRN_ID AS INTEGER) AS Med_Rec
	,ser.PROV_NAME
	,CAST(CASE                
...
FROM CLARITY.dbo.CLARITY_SER ser
			...
			ON physcn.sk_Dim_Physcn = cser.sk_Dim_Physcn
WHERE ser.PROV_TYPE <>'Resource'
	...
	AND (
				...
				AND CAST(lst_cte.Adm_Dt AS DATE) = '8/16/2024'
/*
SELECT
	*
FROM #gpp

--ORDER BY CAST(pt.PAT_ID AS VARCHAR(20)), CAST(ser.PROV_ID AS VARCHAR(30))
		--	,CAST(idx.IDENTITY_ID AS VARCHAR(12))   DESC
ORDER BY PROV_NAME, Atn_Dr, PAT_ID
*/
SELECT
    Atn_Dr_SMS_ID,
    Med_Rec,
