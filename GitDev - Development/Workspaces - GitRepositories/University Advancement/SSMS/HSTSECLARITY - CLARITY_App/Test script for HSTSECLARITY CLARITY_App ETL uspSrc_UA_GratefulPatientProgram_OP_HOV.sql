...
**		CLARITY_App.Rptg.vwDim_Clrt_Pt
...
MODS: 
		**		09/19/2024  -Tom B.  Create stored procedure
				...
				02/28/2025	-Tom B.  Limit future appointments to within 14 days from the current date
				03/05/2025	-Tom B.  Transform discharge datetime value; Add patient, guarantor, and physician identifiers
				09/04/2025	-Tom B.  Exclude encounters that are duplicates of encounters in the IP extract
				09/05/2025	-Tom B.  Edit join to table ACCT_GUAR_PAT_INFO
...
SELECT DISTINCT  --without distinct, results multiple rows becasue diagnosis code in multiple line
	...
	--,hsp.ACCOUNT_ID
	--,gacc.ACCOUNT_ID AS gacc_ACCOUNT_ID
	...
	--,zgrtp.NAME AS GUAR_REL_TO_PAT_NAME
...
LEFT OUTER JOIN CLARITY.dbo.CLARITY_SER ser				ON ser.PROV_ID = hsp.VISIT_PROV_ID 
		...
		END
			...
			LEFT OUTER JOIN IP ip ON ip.PAT_ENC_CSN_ID = hsp.PAT_ENC_CSN_ID

			--LEFT OUTER JOIN CLARITY.dbo.ZC_GUAR_REL_TO_PAT zgrtp ON zgrtp.GUAR_REL_TO_PAT_C = rship.GUAR_REL_TO_PAT_C

WHERE 1 = 1
AND hsp.HSP_ACCOUNT_ID IS NOT NULL
AND ip.PAT_ENC_CSN_ID IS NULL
AND (
			(
...
--	*
--FROM OPHOV
--WHERE OPHOV.PAT_ENC_CSN_ID = 200119053036
...
SELECT
    ...
    ISNULL(CONVERT(VARCHAR(23), CASE WHEN ucinn_ascendv2__Discharge_Date_Time__c IS NOT NULL
			   ...
			   WHEN enc.APPT_STATUS_C IN ('1','2','3','4','5','6','102') -- Scheduled, Completed, Canceled, No Show, Left without seen, Arrived, Walk-in
				AND ucinn_ascendv2__Admit_Date_Time__c < enc.Load_Dtm
...
WHERE 1 = 1
		...
		AND CAST(ucinn_ascendv2__Admit_Date_Time__c AS DATE) <= CAST(DATEADD(DAY,14,GETDATE()) AS DATE)
...
ORDER BY ucinn_ascendv2__External_System_ID__c
END
