...
MODS: 
		**		09/19/2024  -Tom B.  Create stored procedure
				...
				02/26/2025	-Tom B.  Transform discharge datetime
...
SELECT DISTINCT  --without distinct, results multiple rows becasue diagnosis code in multiple line
	...
	,acc.GUARANTOR_ID
	,gacc.ACCOUNT_ID
	,rship.ACCOUNT_ID AS rship_ACCOUNT_ID
...
--  ON x.PAT_ENC_CSN_ID = hsp.PAT_ENC_CSN_ID
		...
		END
			...
			LEFT OUTER JOIN CLARITY..ZC_GUAR_REL_TO_PAT zgrtp ON zgrtp.GUAR_REL_TO_PAT_C = rship.GUAR_REL_TO_PAT_C
...
AND hsp.PAT_ENC_CSN_ID = 200000902026
)
