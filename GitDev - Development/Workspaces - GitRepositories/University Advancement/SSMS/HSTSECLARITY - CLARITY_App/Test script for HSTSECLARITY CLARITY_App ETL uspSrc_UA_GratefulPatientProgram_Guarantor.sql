...
MODS: 
		**		09/19/2024  -Tom B.  Create stored procedure
				...
				09/26/2025	-Tom B.  Correct join to table CLARITY.dbo.ACCT_GUAR_PAT_INFO; filter on UA_GPP_Guar_Rltn_to_Pt values;
														add Red Folder flag; limit first name of guarantor to first 40 characters
...
AND ascrel.ASCEND_VALUE <> 'DNC - Delete the row for these guarantors; we are not importing these consitutents.'
)
...
LEFT OUTER JOIN CLARITY_App.Rptg.ADT_Red_Folder_Extract AS rf
	ON rf.MRN = CAST(pt.PAT_MRN_ID AS INTEGER)
...
SELECT DISTINCT
	...
	CAST(LEFT(CASE
	    WHEN CHARINDEX(',',enc.ACCOUNT_NAME) > 2 THEN
	        ...
	        END
	    ELSE ''
	    END,40) AS VARCHAR(255))																							AS 'ucinn_ascendv2__First_Name__c',
	CAST(CASE
