...
SELECT DISTINCT  --without distinct, results multiple rows becasue diagnosis code in multiple line
	...
	,NULL																												AS	'ucinn_ascendv2__County_Calling_Code_1__c'
	...
	,pt.BIRTH_DATE																								AS	'Health Date of Birth'
	,NULL																												AS	'Health Deceased Date'
	,CAST(CASE WHEN pt4.PAT_LIVING_STAT_C='2' 
			THEN 'D' 
			ELSE '' END  AS VARCHAR(1))																AS	'Health Deceased Status'
...
SELECT DISTINCT
	...
	CAST('UVAH-'+RIGHT('0000000'+CAST(enc.sk_Dim_Pt AS VARCHAR(8)),8)+'-Patient-'+CONVERT(VARCHAR(8),CAST(enc.Load_Dtm AS DATE),112) AS VARCHAR(255)) AS 	'ucinn_ascendv2__External_System_ID__c',
    ...
    CAST(enc.[Is Patient] AS VARCHAR(255))					AS [Is Patient],
	...
	CAST(enc.ucinn_ascendv2__Gender__c AS VARCHAR(255))			AS ucinn_ascendv2__Gender__c,
	enc.[Health Date of Birth]		AS	'Health Date of Birth',
	CAST(enc.[Health Deceased Status] AS VARCHAR(10))		AS 'Health Deceased Status',
    enc.Update_Date,
    enc.Load_Dtm
--FROM #pt enc
FROM pt enc
