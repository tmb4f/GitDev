...
DECLARE @LOC_TO_HOSP TABLE
(
[LOC_ID] NUMERIC(18,0),
[LOC_NAME] VARCHAR(100),
[ASCEND_VALUE] VARCHAR(255)
)

INSERT INTO @LOC_TO_HOSP
(
    LOC_ID,
    LOC_NAME,
    ASCEND_VALUE
...
SELECT DISTINCT  --without distinct, results multiple rows becasue diagnosis code in multiple line
	...
	,ser.PROV_TYPE
	,typ.DISP_ENC_TYPE_C
	,typ.NAME AS DISP_ENC_TYPE_NAME
...
LEFT OUTER JOIN CLARITY.dbo.CLARITY_SER ser				ON ser.PROV_ID = hsp.VISIT_PROV_ID 
			...
			LEFT OUTER JOIN @LOC_TO_HOSP loc		ON loc.LOC_ID = hsp.PRIMARY_LOC_ID
...
SELECT
    OPHOV.ucinn_ascendv2__Department_Formula__c,
    OPHOV.DISP_ENC_TYPE_NAME,
    OPHOV.APPT_STATUS_C,
    OPHOV.PROV_TYPE,
	OPHOV.ucinn_ascendv2__External_System_ID__c,
    OPHOV.sk_Dim_Pt,
    OPHOV.ESI_3,
    OPHOV.ucinn_ascendv2__Guarantor_ID__c,
    OPHOV.ucinn_ascendv2__Provider__c,
    OPHOV.ucinn_ascendv2__Guarantor_Relationship__c,
    OPHOV.ucinn_ascendv2__Appointment_Type__c,
    OPHOV.ucinn_ascendv2__Appointment_Location__c,
    OPHOV.ucinn_ascendv2__Appointment_Status__c,
    OPHOV.ucinn_ascendv2__Data_Source__c,
    OPHOV.ucinn_ascendv2__Admit_Date_Time__c,
    OPHOV.ucinn_ascendv2__Discharge_Date_Time__c,
    OPHOV.ACCT_FIN_CLASS_C,
    OPHOV.ACCT_SLFPYST_HA_C,
    OPHOV.FPL_STATUS_CODE_C,
    OPHOV.FPL_EFF_DATE,
    OPHOV.ACCOUNT_STATUS_C,
    OPHOV.BIRTH_DATE,
    OPHOV.ICD9_CODE,
    OPHOV.Load_Dtm,
    OPHOV.DEPARTMENT_ID,
    OPHOV.HSP_ACCOUNT_ID,
    OPHOV.DISP_ENC_TYPE_C
FROM OPHOV
WHERE OPHOV.ucinn_ascendv2__Department_Formula__c LIKE '%OCCUPATIONAL%'
OR OPHOV.ucinn_ascendv2__Department_Formula__c LIKE '%OCCUPTNL%'
OR OPHOV.ucinn_ascendv2__Department_Formula__c LIKE '%OCCUP%'
ORDER BY
	OPHOV.ucinn_ascendv2__Department_Formula__c,
	OPHOV.DISP_ENC_TYPE_NAME
/*
SELECT
	...
	CAST('UVAH-'+RIGHT('0000000'+CAST(enc.sk_Dim_Pt AS VARCHAR(8)),8)+'-'+ESI_3+'-'+CONVERT(VARCHAR(8),CAST(enc.Load_Dtm AS DATE),112) AS VARCHAR(255)) AS 	ucinn_ascendv2__Patient_Interim__c,
    CAST('UVAH-'+ucinn_ascendv2__Guarantor_ID__c+'-Guarantor-'+CONVERT(VARCHAR(8),CAST(enc.Load_Dtm AS DATE),112)	 AS VARCHAR(255))	AS	ucinn_ascendv2__Guarantor_Interim__c,
    CAST('UVAH-'+enc.ucinn_ascendv2__Provider__c+'-'+CONVERT(VARCHAR(8),CAST(enc.Load_Dtm AS DATE),112) AS VARCHAR(255)) AS 	ucinn_ascendv2__Attending_Interim__c,
...
--ORDER BY	 enc.sk_Dim_Pt, enc.ucinn_ascendv2__Admit_Date_Time__c
*/
END
