...
--CREATE PROCEDURE [ETL].[uspSrc_UA_GratefulPatientProgram_Guarantor] 
...
IF OBJECT_ID('tempdb..#guarpt ') IS NOT NULL
DROP TABLE #guarpt

IF OBJECT_ID('tempdb..#guarname ') IS NOT NULL
DROP TABLE #guarname
...
SELECT DISTINCT  --without distinct, results multiple rows becasue diagnosis code in multiple line
	...
	,pt_typ.PATIENT_TYPE_C
	--,ad1.ADDRESS AS 'Patient_Address_1'
	--,ad2.ADDRESS AS 'Patient_Address_2'
	,gacc.BILLING_ADDRESS_1 AS 'Billing_Address_1'
	,gacc.BILLING_ADDRESS_2 AS 'Billing_Address_2'
	,gacc.CITY AS 'Billing_City'
	,guarst.NAME AS 'Billing_State_Name'
	,guarst.ABBR AS 'Billing_State_Abbr'
	,gacc.ZIP AS 'Billing_Postal_Code'
/*
...
*/
	...
	,gacc.HOME_PHONE AS 'Billing_Home_Phone'
/*
...
*/
	...
	,gacc2.MYPT_ID
	--,pt.EMAIL_ADDRESS AS 'Patient_Email_Address'
	,gacc2.EMAIL_ADDRESS AS 'ACCOUNT_2_Email_Address'
	,mycpt.SECURE_EMAIL
/*
...
WHERE csaseq.updseq = 1
  ...
  ON x.PAT_ENC_CSN_ID = hsp.PAT_ENC_CSN_ID
			...
			LEFT OUTER JOIN CLARITY.dbo.PAT_ENC_HSP peh			ON peh.HSP_ACCOUNT_ID = hsp.HSP_ACCOUNT_ID
																										AND peh.PAT_ENC_CSN_ID = hsp.HSP_ACCOUNT_ID
			...
			INNER JOIN CLARITY.dbo.ACCOUNT_2 gacc2				ON gacc2.ACCT_ID = gacc.ACCOUNT_ID
			...
			LEFT OUTER JOIN CLARITY..MYC_PATIENT mycpt	ON mycpt.MYPT_ID = gacc2.MYPT_ID
...
--ORDER BY gacc.ACCOUNT_NAME
ORDER BY gacc.PAT_REC_OF_GUAR_ID
	
  -- Create index for temp table #ptguar

  CREATE NONCLUSTERED INDEX IX_ptguar ON #ptguar (PAT_REC_OF_GUAR_ID)
...
SELECT
	PAT_REC_OF_GUAR_ID
FROM #ptguar
WHERE PAT_REC_OF_GUAR_ID IS NOT NULL
) gacc
INNER JOIN CLARITY..PATIENT pt
	ON gacc.PAT_REC_OF_GUAR_ID = pt.PAT_ID
LEFT OUTER JOIN CLARITY..ZC_STATE zs
...
SELECT
	guar.ucinn_ascendv2__External_System_ID__c,
    guar.RecordTypeId,
    guar.ucinn_ascendv2__MRN__c,
    guar.ucinn_ascendv2__Donor_ID__c,
    guar.[Is Guarantor],
    guar.[Constituent Type],
    guar.ucinn_ascendv2__Data_Source__c,
    guar.[Name Type],
    guar.ACCOUNT_NAME,
    guar.PAT_REC_OF_GUAR_ID,
    pt.PAT_ID,
    pt.PAT_NAME,
    guar.ucinn_ascendv2__First_Name__c,
    guar.ucinn_ascendv2__Middle_Name__c,
    guar.ucinn_ascendv2__Last_Name__c,
    guar.ucinn_ascendv2__Address_1_Type__c,
    guar.uva_Address_1_Status__c,
    guar.uva_Address_1_Source__c,
    guar.Relationship_to_Patient,
    --guar.Patient_Address_1,
    --guar.Patient_Address_2,
    guar.Billing_Address_1,
    guar.Billing_Address_2,
    guar.Billing_City,
    guar.Billing_State_Name,
    guar.Billing_State_Abbr,
    guar.Billing_Postal_Code,
    pt.ADD_LINE_1,
    pt.ADD_LINE_2,
    pt.CITY,
    pt.STATE_C,
    ...
    pt.STATE_ABBR,
    pt.ZIP,
    guar.ucinn_ascendv2__Phone_1_Type__c,
    guar.uva_Phone_1_Status__c,
    guar.uva_Phone_1_Source__c,
    guar.Billing_Home_Phone,
    pt.HOME_PHONE,
    guar.ucinn_ascendv2__Email_1_Type__c,
    guar.uva_Email_1_Status__c,
    guar.uva_Email_1_Source__c,
    --guar.Patient_Email_Address,
	pt.EMAIL_ADDRESS,
	guar.ACCOUNT_2_Email_Address,
	guar.SECURE_EMAIL,
    guar.ucinn_ascendv2__Relationship_1_Related_Contact__c,
    guar.ucinn_ascendv2__Relationship_1_Related_Role__c,
    guar.ucinn_ascendv2__Relationship_Type_Formula__c,
    guar.ucinn_ascendv2__Is_Self_Guarantor__c,
    guar.ucinn_ascendv2__Admit_Date_Time__c,
    guar.ACCT_FIN_CLASS_C,
    guar.ACCT_SLFPYST_HA_C,
    guar.FPL_STATUS_CODE_C,
    guar.FPL_EFF_DATE,
    guar.ACCOUNT_STATUS_C,
    guar.BIRTH_DATE,
    guar.ICD9_CODE,
    guar.Update_Date,
    guar.Load_Dtm--,
	--pt.PAT_REC_OF_GUAR_ID,
FROM #ptguar guar
LEFT OUTER JOIN #guarpt pt
	ON pt.PAT_REC_OF_GUAR_ID = guar.PAT_REC_OF_GUAR_ID
ORDER BY
	guar.ucinn_ascendv2__External_System_ID__c
/*
SELECT
	gacc.ucinn_ascendv2__External_System_ID__c,
    gacc.RecordTypeId,
    gacc.ucinn_ascendv2__MRN__c,
    gacc.ucinn_ascendv2__Donor_ID__c,
    gacc.[Is Guarantor],
    gacc.[Constituent Type],
    gacc.ucinn_ascendv2__Data_Source__c,
    gacc.[Name Type],
    gacc.ACCOUNT_NAME,
	--pt2.PAT_NAME,
    gacc.PAT_REC_OF_GUAR_ID,
	--pt.PAT_NAME AS GUAR_NAME_FROM_PAT_REC,
    --gacc.PAT_ID,
    gacc.ucinn_ascendv2__First_Name__c,
    gacc.ucinn_ascendv2__Middle_Name__c,
    gacc.ucinn_ascendv2__Last_Name__c,
    gacc.ucinn_ascendv2__Address_1_Type__c,
    gacc.uva_Address_1_Status__c,
    gacc.uva_Address_1_Source__c,
    gacc.ucinn_ascendv2__Address_1_Line_1__c,
    gacc.ucinn_ascendv2__Address_1_Line_2__c,
    gacc.ucinn_ascendv2__City_1__c,
    gacc.ucinn_ascendv2__State_1__c,
    gacc.ucinn_ascendv2__Postal_Code_1__c,
    gacc.ucinn_ascendv2__Phone_1_Type__c,
    gacc.uva_Phone_1_Status__c,
    gacc.uva_Phone_1_Source__c,
    gacc.ucinn_ascendv2__Phone_1__c,
    gacc.ucinn_ascendv2__Email_1_Type__c,
    gacc.uva_Email_1_Status__c,
    gacc.uva_Email_1_Source__c,
    gacc.ucinn_ascendv2__Email_1__c,
    gacc.ucinn_ascendv2__Relationship_1_Related_Contact__c,
    gacc.ucinn_ascendv2__Relationship_1_Related_Role__c,
    gacc.ucinn_ascendv2__Relationship_Type_Formula__c,
    gacc.ucinn_ascendv2__Is_Self_Guarantor__c,
    gacc.ucinn_ascendv2__Admit_Date_Time__c,
    gacc.ACCT_FIN_CLASS_C,
    gacc.ACCT_SLFPYST_HA_C,
    gacc.FPL_STATUS_CODE_C,
    gacc.FPL_EFF_DATE,
    gacc.ACCOUNT_STATUS_C,
    gacc.BIRTH_DATE,
    gacc.ICD9_CODE,
    gacc.Update_Date,
    gacc.Load_Dtm
...
LEFT OUTER JOIN CLARITY.dbo.PATIENT pt
	ON pt.PAT_ID = gacc.PAT_REC_OF_GUAR_ID
----LEFT OUTER JOIN CLARITY.dbo.PATIENT pt2
----	ON pt.PAT_NAME = gacc.ACCOUNT_NAME
ORDER BY gacc.ACCOUNT_NAME
	
  -- Create index for temp table #guarname

--  CREATE NONCLUSTERED INDEX IX_ACCOUNT_NAME ON #guarname (ACCOUNT_NAME)
...
--LEFT OUTER JOIN CLARITY.dbo.PATIENT pt
--	ON pt.PAT_NAME = gacc.ACCOUNT_NAME
--WHERE gacc.PAT_REC_OF_GUAR_ID IS NULL
--ORDER BY gacc.ACCOUNT_NAME
*/
/*
SELECT DISTINCT
