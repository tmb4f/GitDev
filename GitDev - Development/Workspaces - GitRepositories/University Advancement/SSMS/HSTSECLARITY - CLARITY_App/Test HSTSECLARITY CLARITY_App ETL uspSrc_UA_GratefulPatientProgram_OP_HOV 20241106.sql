...
SELECT DISTINCT  --without distinct, results multiple rows becasue diagnosis code in multiple line
	...
	--,hsp.PAT_ENC_CSN_ID
	,clpt.sk_Dim_Pt
	--,RIGHT('0000000'+CAST(clpt.sk_Dim_Pt AS VARCHAR(8)),8)						AS	'ucinn_ascendv2__Patient_ID__c'
	...
	--,ser.PROV_ID																									AS	'ucinn_ascendv2__Attending_Physician__c'
	...
	--,CASE WHEN hsp.HOSP_DISCHRG_TIME IS NOT NULL THEN 'TRUE' ELSE 'FALSE' END	AS 'ucinn_ascendv2__Discharge_Flag__c'
...
SELECT
	--CAST('UVAH-'+RIGHT('000000000000'+CAST(enc.PAT_ENC_CSN_ID AS VARCHAR(12)),12)+'-Encounter-'+CONVERT(VARCHAR(8),CAST(enc.Load_Dtm AS DATE),112) AS VARCHAR(255)) AS 	'ucinn_ascendv2__External_System_ID__c',
	...
	'UVAH-'+ucinn_ascendv2__Guarantor_ID__c+'-Guarantor-'+CONVERT(VARCHAR(8),CAST(enc.Load_Dtm AS DATE),112)	AS	'ucinn_ascendv2__Guarantor_Interim__c',
