...
SELECT -- IP
	...
	,NULL AS PRC_NAME
	,hsp2.ADMISSION_PROV_ID AS VISIT_PROV_ID
	,hsp2.HOSP_DISCH_TIME AS HOSP_DISCHRG_TIME
	,acc.ADM_LOC_ID AS PRIMARY_LOC_ID
	,'IP' AS enc_class
	,hsp2.DEPARTMENT_ID
	,hsp2.HSP_ACCOUNT_ID
FROM  CLARITY.dbo.PAT_ENC_HSP hsp2												
LEFT OUTER JOIN CLARITY.dbo.HSP_ACCOUNT acc				ON acc.HSP_ACCOUNT_ID = hsp2.HSP_ACCOUNT_ID
WHERE 1 = 1
AND hsp2.HSP_ACCOUNT_ID IS NOT NULL
AND (
			((hsp2.CONTACT_DATE >=@StartDate		AND		hsp2.CONTACT_DATE<@EndDate)
			 OR 
			 (hsp2.HOSP_ADMSN_TIME  >=@StartDate	AND		hsp2.HOSP_ADMSN_TIME <@EndDate)
			)
		)
)
...
SELECT DISTINCT  --without distinct, results multiple rows becasue diagnosis code in multiple line
	...
	,encs.PRC_NAME 'Hospital_Visit_Name'
	,appt.APPT_DTTM
	,hsp.CONTACT_DATE
	,hsp.HOSP_ADMSN_TIME
	...
	,encs.HOSP_DISCHRG_TIME 'Discharge_Date/Time'
...
INNER JOIN enc_cte encs		ON hsp.PAT_ENC_CSN_ID = encs.PAT_ENC_CSN_ID
...
--  ON x.PAT_ENC_CSN_ID = hsp.PAT_ENC_CSN_ID
			...
			LEFT OUTER JOIN CLARITY.dbo.CLARITY_DEP dep				ON dep.DEPARTMENT_ID = encs.DEPARTMENT_ID
			...
			LEFT OUTER JOIN CLARITY..CLARITY_LOC loc		ON loc.LOC_ID = encs.PRIMARY_LOC_ID
			...
			ON mdm.EPIC_DEPARTMENT_ID = encs.DEPARTMENT_ID

			--LEFT JOIN [CLARITY_App].[Mapping].[Epic_Dept_Groupers] g ON hsp.DEPARTMENT_ID = g.epic_department_id
			LEFT JOIN [CLARITY_App].[Mapping].[Epic_Dept_Groupers] g ON encs.DEPARTMENT_ID = g.epic_department_id
...
ORDER BY	encs.enc_class, pt.PAT_ID, hsp.PAT_ENC_CSN_ID, ser.PROV_ID
END
/*
