...
SELECT DISTINCT  --without distinct, results multiple rows becasue diagnosis code in multiple line
	...
	--,guarst.ABBR AS gacc_ABBR
	,gacc.STATE_C AS gacc_STATE_C
	--,CAST(guarst.ABBR AS VARCHAR (255))														AS	'ucinn_ascendv2__State_1__c'
	...
	,rel.NAME																										AS	'ucinn_ascendv2__Relationship_1_Related_Role__c'
...
WHERE csaseq.updseq = 1
  ...
  ON x.PAT_ENC_CSN_ID = hsp.PAT_ENC_CSN_ID
	AND hsp.CONTACT_DATE >= '10/18/2024 00:00'
...
ORDER BY
  ...
  CREATE CLUSTERED INDEX IX_ptguar ON #ptguar (PAT_ID, PAT_REC_OF_GUAR_ID, ucinn_ascendv2__Admit_Date_Time__c)
...
SELECT
     ...
     enc.gacc_BILLING_ADDRESS_1,
     enc.gacc_BILLING_ADDRESS_2,
     enc.gacc_CITY,
     enc.gacc_STATE_C,
     enc.gacc_ZIP,
	...
	--'UVAH-'+TRIM(pt.PAT_MRN_ID)																		AS	'ucinn_ascendv2__External_System_ID__c'
	'UVAH-'+RIGHT('0000000'+CAST(clpt.sk_Dim_Pt AS VARCHAR(8)),8)+'-Guarantor-'+CONVERT(VARCHAR(8),CAST(enc.Load_Dtm AS DATE),112)	AS	'ucinn_ascendv2__External_System_ID__c'
	--,pt.PAT_MRN_ID																								AS	'ucinn_ascendv2__MRN__c'
	,RIGHT('0000000'+CAST(clpt.sk_Dim_Pt AS VARCHAR(8)),8)						AS	'ucinn_ascendv2__MRN__c'
	,guar.ADD_LINE_1
	,guar.ADD_LINE_2
	,CAST(COALESCE(guar.ADD_LINE_1,enc.gacc_BILLING_ADDRESS_1)			AS VARCHAR(255))   'ucinn_ascendv2__Address_1_Line_1__c'
	,CAST(COALESCE(guar.ADD_LINE_2,enc.gacc_BILLING_ADDRESS_2)			AS VARCHAR(255))   'ucinn_ascendv2__Address_1_Line_2__c'
	,guar.CITY	AS guar_CITY
	,CAST(COALESCE(guar.CITY,enc.gacc_CITY) AS VARCHAR(255))			AS	'ucinn_ascendv2__City_1__c'
	,guar.STATE_C AS guar_STATE_C
	,COALESCE(guar.STATE_C,enc.gacc_STATE_C) AS STATE_C
	,guar.ZIP	AS guar_ZIP
	,CAST(COALESCE(guar.ZIP,enc.gacc_ZIP) AS VARCHAR (40))							AS	'ucinn_ascendv2__Postal_Code_1__c'
	,guar.HOME_PHONE	AS guar_HOME_PHONE
	,CAST(COALESCE(guar.HOME_PHONE,enc.gacc_HOME_PHONE) AS VARCHAR(15))	AS	'ucinn_ascendv2__Phone_1__c'
	,guar.EMAIL_ADDRESS	AS guar_EMAIL_ADDRESS
	,CAST(COALESCE(guar.EMAIL_ADDRESS,enc.gacc2_EMAIL_ADDRESS,enc.mycpt_EMAIL_ADDRESS)	 AS VARCHAR(70))	 AS	'ucinn_ascendv2__Email_1__c'
	,'UVAH-'+RIGHT('0000000'+CAST(clpt.sk_Dim_Pt AS VARCHAR(8)),8)+'-Patient-'+CONVERT(VARCHAR(8),CAST(enc.Load_Dtm AS DATE),112)	AS	'ucinn_ascendv2__Relationship_1_Related_Contact__c'
FROM #ptguar enc
