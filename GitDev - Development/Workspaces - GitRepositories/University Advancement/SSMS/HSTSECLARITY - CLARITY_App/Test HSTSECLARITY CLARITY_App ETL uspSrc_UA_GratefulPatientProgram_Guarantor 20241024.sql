...
SELECT DISTINCT  --without distinct, results multiple rows becasue diagnosis code in multiple line
	...
	,gacc.BILLING_ADDRESS_1
	,gacc.BILLING_ADDRESS_2
	,guar.ADD_LINE_1
	,guar.ADD_LINE_2
	,CAST(COALESCE(guar.ADD_LINE_1,gacc.BILLING_ADDRESS_1)			AS VARCHAR(255))   'ucinn_ascendv2__Address_1_Line_1__c'
	,CAST(COALESCE(guar.ADD_LINE_2,gacc.BILLING_ADDRESS_2)			AS VARCHAR(255))   'ucinn_ascendv2__Address_1_Line_2__c'
	,gacc.CITY	AS gacc_CITY
	,guar.CITY	AS guar_CITY
	,CAST(COALESCE(guar.CITY,gacc.CITY) AS VARCHAR(255))					AS	'ucinn_ascendv2__City_1__c'
	,guarst.NAME	AS	gacc_STATE_NAME
	,guarst.ABBR	AS	gacc_STATE_ABBR
	,guar.STATE_NAME	AS guar_STATE_NAME
	,guar.STATE_ABBR	AS guar_STATE_ABBR
	,CAST(COALESCE(guar.STATE_ABBR,guarst.ABBR) AS VARCHAR (255))	AS	'ucinn_ascendv2__State_1__c'
	,gacc.ZIP	AS gacc_ZIP
	,guar.ZIP	AS guar_ZIP
	,CAST(COALESCE(guar.ZIP,gacc.ZIP) AS VARCHAR (40))							AS	'ucinn_ascendv2__Postal_Code_1__c'
	...
	,guar.HOME_PHONE	AS guar_HOME_PHONE
	,CAST(COALESCE(guar.HOME_PHONE,gacc.HOME_PHONE) AS VARCHAR(15))	AS	'ucinn_ascendv2__Phone_1__c'
	...
	,guar.EMAIL_ADDRESS AS guar_EMAIL_ADDRESS
	,mycpt.SECURE_EMAIL	AS mycpt_EMAIL_ADDRESS
	,CAST(COALESCE(guar.EMAIL_ADDRESS,gacc2.EMAIL_ADDRESS) AS VARCHAR(70))	AS 'ucinn_ascendv2__Email_1__c'
...
WHERE csaseq.updseq = 1
  ...
  ON x.PAT_ENC_CSN_ID = hsp.PAT_ENC_CSN_ID
			...
			LEFT OUTER JOIN Rptg.ADT_Red_Folder_Extract Rd				ON rd.Acc_ID=acc.HSP_ACCOUNT_ID
			...
			LEFT OUTER JOIN CLARITY.dbo.HSP_ACCT_ATND_PROV	haatn ON haatn.HSP_ACCOUNT_ID = acc.HSP_ACCOUNT_ID		-- Admitting Provider
									AND	   haatn.LINE = 1

			LEFT OUTER JOIN CLARITY.dbo.CLARITY_PRC prc	ON prc.PRC_ID = hsp.APPT_PRC_ID

			LEFT OUTER JOIN CLARITY.dbo.V_SCHED_APPT appt	ON appt.PAT_ENC_CSN_ID = hsp.PAT_ENC_CSN_ID

			LEFT OUTER JOIN CLARITY.dbo.CLARITY_LOC loc		ON loc.LOC_ID = hsp.PRIMARY_LOC_ID

			LEFT OUTER JOIN CLARITY.dbo.CLARITY_EPM payor	ON payor.PAYOR_ID = acc.PRIMARY_PAYOR_ID

			LEFT OUTER JOIN CLARITY.dbo.ZC_FIN_CLASS zfc	ON zfc.FIN_CLASS_C = payor.FINANCIAL_CLASS

			LEFT OUTER JOIN CLARITY.dbo.PAT_ENC_3 enc3	ON enc3.PAT_ENC_CSN = hsp.PAT_ENC_CSN_ID

			LEFT OUTER JOIN CLARITY.dbo.CLARITY_EPP epp	ON epp.BENEFIT_PLAN_ID = acc.PRIMARY_PLAN_ID
...
SELECT
	enc.ACCOUNT_ID,
	enc.ucinn_ascendv2__External_System_ID__c,
    enc.RecordTypeId,
    enc.ucinn_ascendv2__MRN__c,
    enc.ucinn_ascendv2__Donor_ID__c,
    enc.[Is Guarantor],
    enc.[Constituent Type],
    enc.ucinn_ascendv2__Data_Source__c,
    enc.[Name Type],
	enc.PATIENT_TYPE_C,
	enc.PATIENT_TYPE_NAME,
    enc.ucinn_ascendv2__First_Name__c,
    enc.ucinn_ascendv2__Middle_Name__c,
    enc.ucinn_ascendv2__Last_Name__c,
    enc.ucinn_ascendv2__Address_1_Type__c,
    enc.uva_Address_1_Status__c,
    enc.uva_Address_1_Source__c,
	enc.BILLING_ADDRESS_1,
	enc.BILLING_ADDRESS_2,
    enc.ADD_LINE_1,
    enc.ADD_LINE_2,
    enc.ucinn_ascendv2__Address_1_Line_1__c,
    enc.ucinn_ascendv2__Address_1_Line_2__c,
    enc.gacc_CITY,
	enc.guar_CITY,
    enc.ucinn_ascendv2__City_1__c,
    enc.gacc_STATE_ABBR,
    enc.guar_STATE_ABBR,
    enc.ucinn_ascendv2__State_1__c,
	enc.gacc_ZIP,
	enc.guar_ZIP,
    enc.ucinn_ascendv2__Postal_Code_1__c,
    enc.ucinn_ascendv2__Phone_1_Type__c,
    enc.uva_Phone_1_Status__c,
    enc.uva_Phone_1_Source__c,
    enc.gacc_HOME_PHONE,
    enc.guar_HOME_PHONE,
    enc.ucinn_ascendv2__Phone_1__c,
    enc.ucinn_ascendv2__Email_1_Type__c,
    enc.uva_Email_1_Status__c,
    enc.uva_Email_1_Source__c,
    enc.gacc2_EMAIL_ADDRESS,
    enc.guar_EMAIL_ADDRESS,
	enc.mycpt_EMAIL_ADDRESS,
    enc.ucinn_ascendv2__Email_1__c,
    enc.ucinn_ascendv2__Relationship_1_Related_Contact__c,
    enc.ucinn_ascendv2__Relationship_1_Related_Role__c,
    enc.ucinn_ascendv2__Relationship_Type_Formula__c,
    enc.ucinn_ascendv2__Is_Self_Guarantor__c,
...
FROM ptguar enc
ORDER BY enc.ucinn_ascendv2__MRN__c
/*
SELECT DISTINCT
