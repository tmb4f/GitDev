...
SELECT DISTINCT  --without distinct, results multiple rows becasue diagnosis code in multiple line
	...
	,COALESCE(ptot.PROV_TYPE_OT_NAME, ptot2.PROV_TYPE_OT_NAME) AS PROV_TYPE_OT_NAME
...
LEFT OUTER JOIN CLARITY.dbo.ZC_HOSP_ADMSN_TYPE zhat				ON zhat.HOSP_ADMSN_TYPE_C = hsp.HOSP_ADMSN_TYPE_C		
			...
			--	ON ser.PROV_ID = ptot.PROV_ID
			--	AND pe.ADMSN_TIME BETWEEN ptot.CONTACT_DATE AND ptot.EFF_TO_DATE	
				
			LEFT OUTER JOIN CLARITY_App.Rptg.vwCLARITY_SER_OT_PROV_TYPE AS ptot
				ON ser.PROV_ID = ptot.PROV_ID
				AND pe.UPDATE_DATE BETWEEN ptot.CONTACT_DATE AND ptot.EFF_TO_DATE
				
			LEFT OUTER JOIN CLARITY_App.Rptg.vwCLARITY_SER_OT_PROV_TYPE AS ptot2
				ON ser.PROV_ID = ptot2.PROV_ID
				AND pe.ADMSN_TIME BETWEEN ptot2.CONTACT_DATE AND ptot2.EFF_TO_DATE
