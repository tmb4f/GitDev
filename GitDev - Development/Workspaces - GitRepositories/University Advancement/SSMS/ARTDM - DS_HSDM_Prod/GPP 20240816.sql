...
SET @BeginDt = CONVERT(DATETIME,@strBegindt)
...
SELECT 
       ...
       cte.Atn_Dr_MI ,
       cte.Atn_Dr_LName ,
       UPPER(cte.Atn_Dr_FName) AS Atn_Dr_FName_uc ,
       UPPER(cte.Atn_Dr_MI) AS Atn_Dr_MI_uc ,
       UPPER(cte.Atn_Dr_LName) AS Atn_Dr_LName_uc  ,
       cte.Atn_Dr_Type ,
       cte.Atn_Dr_Status ,
...
ON physcn.IDNumber = cte.Atn_Dr

WHERE cte.Med_Rec <> 0
  AND pt_typ.PATIENT_TYPE_C IS NULL --not a prisoner/inmate
  AND CAST(cte.Adm_Dt AS DATE) = '8/16/2024'

ORDER BY cte.DisplayName, cte.Atn_Dr
       , cte.Med_Rec
*/
SELECT 
       cte.Atn_Dr ,
